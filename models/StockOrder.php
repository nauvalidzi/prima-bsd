...
/**
 * Table class for stock_order
 */
class StockOrder extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'stock_order';
        $this->TableName = 'stock_order';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`stock_order`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id
        $this->id = new DbField('stock_order', 'stock_order', 'x_id', 'id', '`id`', '`id`', 20, 20, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // tanggal
        $this->tanggal = new DbField('stock_order', 'stock_order', 'x_tanggal', 'tanggal', '`tanggal`', CastDateFieldForLike("`tanggal`", 7, "DB"), 133, 10, 7, false, '`tanggal`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // idpegawai
        $this->idpegawai = new DbField('stock_order', 'stock_order', 'x_idpegawai', 'idpegawai', '`idpegawai`', '`idpegawai`', 3, 11, -1, false, '`idpegawai`', false, false, false, 'FORMATTED TEXT', 'SELECT');
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "stock_order_detail") {
            $detailUrl = Container("stock_order_detail")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "StockOrderList";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`stock_order`";
    }
    ...
    {
        // Cascade Update detail table 'stock_order_detail'
        $cascadeUpdate = false;
        $rscascade = [];
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("stock_order_detail")->loadRs("`pid` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB'))->fetchAll(\PDO::FETCH_ASSOC);
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("stock_order_detail")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("stock_order_detail")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("stock_order_detail")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'stock_order_detail'
        $dtlrows = Container("stock_order_detail")->loadRs("`pid` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("stock_order_detail")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("stock_order_detail")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("stock_order_detail")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("StockOrderList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "StockOrderEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "StockOrderAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            ...
            case Config("API_ADD_ACTION"):
                return "StockOrderAdd";
            case Config("API_EDIT_ACTION"):
                return "StockOrderEdit";
            case Config("API_DELETE_ACTION"):
                return "StockOrderDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("StockOrderView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("StockOrderView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "StockOrderAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "StockOrderAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("StockOrderEdit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("StockOrderEdit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("StockOrderAdd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("StockOrderAdd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("StockOrderDelete", $this->getUrlParm());
    }
