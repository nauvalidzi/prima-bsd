...
/**
 * Table class for deliveryorder
 */
class Deliveryorder extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'deliveryorder';
        $this->TableName = 'deliveryorder';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`deliveryorder`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id
        $this->id = new DbField('deliveryorder', 'deliveryorder', 'x_id', 'id', '`id`', '`id`', 20, 20, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // kode
        $this->kode = new DbField('deliveryorder', 'deliveryorder', 'x_kode', 'kode', '`kode`', '`kode`', 200, 50, -1, false, '`kode`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // tanggal
        $this->tanggal = new DbField('deliveryorder', 'deliveryorder', 'x_tanggal', 'tanggal', '`tanggal`', CastDateFieldForLike("`tanggal`", 0, "DB"), 135, 19, 0, false, '`tanggal`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // lampiran
        $this->lampiran = new DbField('deliveryorder', 'deliveryorder', 'x_lampiran', 'lampiran', '`lampiran`', '`lampiran`', 200, 255, -1, true, '`lampiran`', false, false, false, 'FORMATTED TEXT', 'FILE');
        ...
        // created_at
        $this->created_at = new DbField('deliveryorder', 'deliveryorder', 'x_created_at', 'created_at', '`created_at`', CastDateFieldForLike("`created_at`", 0, "DB"), 135, 19, 0, false, '`created_at`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // updated_at
        $this->updated_at = new DbField('deliveryorder', 'deliveryorder', 'x_updated_at', 'updated_at', '`updated_at`', CastDateFieldForLike("`updated_at`", 0, "DB"), 135, 19, 0, false, '`updated_at`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "deliveryorder_detail") {
            $detailUrl = Container("deliveryorder_detail")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "DeliveryorderList";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`deliveryorder`";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("DeliveryorderList");
    }
    ...
    {
        global $Language;
        if ($pageName == "DeliveryorderView") {
            return $Language->phrase("View");
        } elseif ($pageName == "DeliveryorderEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "DeliveryorderAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            ...
            case Config("API_ADD_ACTION"):
                return "DeliveryorderAdd";
            case Config("API_EDIT_ACTION"):
                return "DeliveryorderEdit";
            case Config("API_DELETE_ACTION"):
                return "DeliveryorderDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("DeliveryorderView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("DeliveryorderView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "DeliveryorderAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "DeliveryorderAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("DeliveryorderEdit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("DeliveryorderEdit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("DeliveryorderAdd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("DeliveryorderAdd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("DeliveryorderDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function getUserIDSubquery(&$fld, &$masterfld)
    {
        global $UserTable;
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM `deliveryorder`";
    ...
    public function rowDeleting(&$rs)
    {
        ...
        // replicate children deleting actions:
        $query = ExecuteQuery("SELECT * from deliveryorder_detail WHERE iddeliveryorder=".$rs['id']);
        $children = $query->fetchAll();
        foreach ($children as $dod) {
        	$myResult = ExecuteUpdate("UPDATE order_detail SET sisa=sisa+(".$dod['jumlahkirim'].") WHERE id=".$dod['idorder_detail']);

        	// update stock
        	addStock($dod['idorder_detail'], -($dod['jumlahkirim']));

        	// check order
            checkCloseOrder($dod['idorder_detail']);
        }

        // Delete related children
        $myResult = ExecuteUpdate("DELETE FROM deliveryorder_detail WHERE iddeliveryorder=".$rs['id']);
        foreach ($children as $dod) {
            // check readOnly setiap order detail dan order
            checkReadOnly("order_detail", $dod['idorder_detail']);
            checkReadOnly("order", $dod['idorder']);
        }
        return true;
    }
