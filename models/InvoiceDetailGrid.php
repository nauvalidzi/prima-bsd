 ...
 * Page class
 */
class InvoiceDetailGrid extends InvoiceDetail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "InvoiceDetailGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "finvoice_detailgrid";
    ...
    public function run()
    {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
                if ($this->getCurrentMasterTable() == "invoice") {
                    $this->DbMasterFilter = $this->addMasterUserIDFilter($this->DbMasterFilter, "invoice"); // Add master User ID filter
                }
        }
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "invoice") {
            $masterTbl = Container("invoice");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("InvoiceList"); // Return to master page
                return;
            } else {
    ...
    {
        $this->diskonpayment->FormValue = ""; // Clear form value
        $this->bbpersen->FormValue = ""; // Clear form value
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_jumlahorder") && $CurrentForm->hasValue("o_jumlahorder") && $this->jumlahorder->CurrentValue != $this->jumlahorder->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_bonus") && $CurrentForm->hasValue("o_bonus") && $this->bonus->CurrentValue != $this->bonus->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_stockdo") && $CurrentForm->hasValue("o_stockdo") && $this->stockdo->CurrentValue != $this->stockdo->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_jumlahkirim") && $CurrentForm->hasValue("o_jumlahkirim") && $this->jumlahkirim->CurrentValue != $this->jumlahkirim->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_jumlahbonus") && $CurrentForm->hasValue("o_jumlahbonus") && $this->jumlahbonus->CurrentValue != $this->jumlahbonus->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_harga") && $CurrentForm->hasValue("o_harga") && $this->harga->CurrentValue != $this->harga->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_totalnondiskon") && $CurrentForm->hasValue("o_totalnondiskon") && $this->totalnondiskon->CurrentValue != $this->totalnondiskon->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_diskonpayment") && $CurrentForm->hasValue("o_diskonpayment") && $this->diskonpayment->CurrentValue != $this->diskonpayment->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_bbpersen") && $CurrentForm->hasValue("o_bbpersen") && $this->bbpersen->CurrentValue != $this->bbpersen->OldValue) {
            return false;
        }
    ...
    public function resetFormError()
    {
        ...
        $this->jumlahorder->clearErrorMessage();
        $this->bonus->clearErrorMessage();
        $this->stockdo->clearErrorMessage();
        $this->jumlahkirim->clearErrorMessage();
        $this->jumlahbonus->clearErrorMessage();
        $this->harga->clearErrorMessage();
        $this->totalnondiskon->clearErrorMessage();
        $this->diskonpayment->clearErrorMessage();
        $this->bbpersen->clearErrorMessage();
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_idorder_detail")) {
        ...
        if ($CurrentForm->hasValue("o_jumlahorder")) {
            $this->jumlahorder->setOldValue($CurrentForm->getValue("o_jumlahorder"));
        }
        ...
        if ($CurrentForm->hasValue("o_stockdo")) {
            $this->stockdo->setOldValue($CurrentForm->getValue("o_stockdo"));
        }
        ...
        if ($CurrentForm->hasValue("o_jumlahkirim")) {
            $this->jumlahkirim->setOldValue($CurrentForm->getValue("o_jumlahkirim"));
        }
        ...
        if ($CurrentForm->hasValue("o_jumlahbonus")) {
            $this->jumlahbonus->setOldValue($CurrentForm->getValue("o_jumlahbonus"));
        }
        ...
        if ($CurrentForm->hasValue("o_totalnondiskon")) {
            $this->totalnondiskon->setOldValue($CurrentForm->getValue("o_totalnondiskon"));
        }
        ...
        if ($CurrentForm->hasValue("o_diskonpayment")) {
            $this->diskonpayment->setOldValue($CurrentForm->getValue("o_diskonpayment"));
        }
        ...
        if ($CurrentForm->hasValue("o_bbpersen")) {
            $this->bbpersen->setOldValue($CurrentForm->getValue("o_bbpersen"));
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (strval($this->diskonpayment->EditValue) != "" && is_numeric($this->diskonpayment->EditValue)) {
                ...
                $this->diskonpayment->OldValue = $this->diskonpayment->EditValue;
            }
            ...
            if (strval($this->bbpersen->EditValue) != "" && is_numeric($this->bbpersen->EditValue)) {
                ...
                $this->bbpersen->OldValue = $this->bbpersen->EditValue;
            }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // idorder_detail
            $this->idorder_detail->setDbValueDef($rsnew, $this->idorder_detail->CurrentValue, 0, $this->idorder_detail->ReadOnly);
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "invoice") {
            $this->idinvoice->CurrentValue = $this->idinvoice->getSessionValue();
        }
        $conn = $this->getConnection();
