...
/**
 * Table class for faktur
 */
class Faktur extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'faktur';
        $this->TableName = 'faktur';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`faktur`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id
        $this->id = new DbField('faktur', 'faktur', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'NO');
        ...
        // idorder
        $this->idorder = new DbField('faktur', 'faktur', 'x_idorder', 'idorder', '`idorder`', '`idorder`', 3, 11, -1, false, '`idorder`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        // tanggal
        $this->tanggal = new DbField('faktur', 'faktur', 'x_tanggal', 'tanggal', '`tanggal`', CastDateFieldForLike("`tanggal`", 7, "DB"), 133, 10, 7, false, '`tanggal`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // kode
        $this->kode = new DbField('faktur', 'faktur', 'x_kode', 'kode', '`kode`', '`kode`', 200, 50, -1, false, '`kode`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // tagihan
        $this->tagihan = new DbField('faktur', 'faktur', 'x_tagihan', 'tagihan', '`tagihan`', '`tagihan`', 3, 15, -1, false, '`tagihan`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // piutang
        $this->piutang = new DbField('faktur', 'faktur', 'x_piutang', 'piutang', '`piutang`', '`piutang`', 3, 15, -1, false, '`piutang`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // updated_at
        $this->updated_at = new DbField('faktur', 'faktur', 'x_updated_at', 'updated_at', '`updated_at`', CastDateFieldForLike("`updated_at`", 11, "DB"), 135, 19, 11, false, '`updated_at`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`faktur`";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("FakturList");
    }
    ...
    {
        global $Language;
        if ($pageName == "FakturView") {
            return $Language->phrase("View");
        } elseif ($pageName == "FakturEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "FakturAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("FakturView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("FakturView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "FakturAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "FakturAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("FakturEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("FakturAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("FakturDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        $curVal = trim(strval($this->idorder->CurrentValue));
        if ($curVal != "") {
            $this->idorder->EditValue = $this->idorder->lookupCacheOption($curVal);
            if ($this->idorder->EditValue === null) { // Lookup from database
                $filterWrk = "`id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                $sqlWrk = $this->idorder->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->idorder->Lookup->renderViewRow($rswrk[0]);
                    $this->idorder->EditValue = $this->idorder->displayValue($arwrk);
                } else {
                    $this->idorder->EditValue = $this->idorder->CurrentValue;
                }
            }
        } else {
            $this->idorder->EditValue = null;
        }
        $this->idorder->ViewCustomAttributes = "";
