 ...
 * Page class
 */
class IjinbpomList extends Ijinbpom
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "IjinbpomList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fijinbpomlist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "IjinbpomAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "IjinbpomDelete";
        $this->MultiUpdateUrl = "IjinbpomUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fijinbpomlistsrch";
    ...
    public function run()
    {
        ...
        $this->suratkuasa->Visible = false;
        $this->suratpembagian->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->kontrakkerjasama->AdvancedSearch->toJson(), ","); // Field kontrakkerjasama
        $filterList = Concat($filterList, $this->suratkuasa->AdvancedSearch->toJson(), ","); // Field suratkuasa
        $filterList = Concat($filterList, $this->suratpembagian->AdvancedSearch->toJson(), ","); // Field suratpembagian
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fijinbpomlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field kontrakkerjasama
        $this->kontrakkerjasama->AdvancedSearch->SearchValue = @$filter["x_kontrakkerjasama"];
        $this->kontrakkerjasama->AdvancedSearch->SearchOperator = @$filter["z_kontrakkerjasama"];
        $this->kontrakkerjasama->AdvancedSearch->SearchCondition = @$filter["v_kontrakkerjasama"];
        $this->kontrakkerjasama->AdvancedSearch->SearchValue2 = @$filter["y_kontrakkerjasama"];
        $this->kontrakkerjasama->AdvancedSearch->SearchOperator2 = @$filter["w_kontrakkerjasama"];
        $this->kontrakkerjasama->AdvancedSearch->save();

        // Field suratkuasa
        $this->suratkuasa->AdvancedSearch->SearchValue = @$filter["x_suratkuasa"];
        $this->suratkuasa->AdvancedSearch->SearchOperator = @$filter["z_suratkuasa"];
        $this->suratkuasa->AdvancedSearch->SearchCondition = @$filter["v_suratkuasa"];
        $this->suratkuasa->AdvancedSearch->SearchValue2 = @$filter["y_suratkuasa"];
        $this->suratkuasa->AdvancedSearch->SearchOperator2 = @$filter["w_suratkuasa"];
        $this->suratkuasa->AdvancedSearch->save();

        // Field suratpembagian
        $this->suratpembagian->AdvancedSearch->SearchValue = @$filter["x_suratpembagian"];
        $this->suratpembagian->AdvancedSearch->SearchOperator = @$filter["z_suratpembagian"];
        $this->suratpembagian->AdvancedSearch->SearchCondition = @$filter["v_suratpembagian"];
        $this->suratpembagian->AdvancedSearch->SearchValue2 = @$filter["y_suratpembagian"];
        $this->suratpembagian->AdvancedSearch->SearchOperator2 = @$filter["w_suratpembagian"];
        $this->suratpembagian->AdvancedSearch->save();
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->kontrakkerjasama, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->suratkuasa, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->suratpembagian, $arKeywords, $type);
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->kontrakkerjasama->setSort("");
                $this->suratkuasa->setSort("");
                $this->suratpembagian->setSort("");
                $this->status->setSort("");
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_ijinbpom_detail"
        $item = &$this->ListOptions->add("detail_ijinbpom_detail");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'ijinbpom_detail') && !$this->ShowMultipleDetails;
        $item->OnLeft = false;
    ...
    public function renderListOptions()
    {
        ...
        // "detail_ijinbpom_detail"
        $opt = $this->ListOptions["detail_ijinbpom_detail"];
        if ($Security->allowList(CurrentProjectID() . 'ijinbpom_detail') && $this->showOptionLink()) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("ijinbpom_detail", "TblCaption");
            $body .= "&nbsp;" . str_replace("%c", Container("ijinbpom_detail")->Count, $Language->phrase("DetailCount"));
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("IjinbpomDetailList?" . Config("TABLE_SHOW_MASTER") . "=ijinbpom&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            ...
            if ($detailPage->DetailView && $Security->canView() && $this->showOptionLink("view") && $Security->allowView(CurrentProjectID() . 'ijinbpom')) {
                ...
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=ijinbpom_detail");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "ijinbpom_detail";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $this->showOptionLink("edit") && $Security->allowEdit(CurrentProjectID() . 'ijinbpom')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=ijinbpom_detail");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
                $detailEditTblVar .= "ijinbpom_detail";
            }
            if ($links != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                $item = &$option->add("detailadd_ijinbpom_detail");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=ijinbpom_detail");
                $detailPage = Container("IjinbpomDetailGrid");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fijinbpomlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fijinbpomlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fijinbpomlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function renderListOptionsExt()
    {
        ...
        // Column "detail_ijinbpom_detail"
        if ($this->DetailPages && $this->DetailPages["ijinbpom_detail"] && $this->DetailPages["ijinbpom_detail"]->Visible) {
            $link = "";
            $option = $this->ListOptions["detail_ijinbpom_detail"];
            $url = "IjinbpomDetailPreview?t=ijinbpom&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"ijinbpom_detail\" data-url=\"" . $url . "\">";
            if ($Security->allowList(CurrentProjectID() . 'ijinbpom')) {
                $label = $Language->TablePhrase("ijinbpom_detail", "TblCaption");
                $label .= "&nbsp;" . JsEncode(str_replace("%c", Container("ijinbpom_detail")->Count, $Language->phrase("DetailCount")));
                $link = "<li class=\"nav-item\"><a href=\"#\" class=\"nav-link\" data-toggle=\"tab\" data-table=\"ijinbpom_detail\" data-url=\"" . $url . "\">" . $label . "</a></li>";
                $links .= $link;
                $detaillnk = JsEncodeAttribute("IjinbpomDetailList?" . Config("TABLE_SHOW_MASTER") . "=ijinbpom&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
                $btngrp .= "<a href=\"#\" class=\"mr-2\" title=\"" . $Language->TablePhrase("ijinbpom_detail", "TblCaption") . "\" onclick=\"window.location='" . $detaillnk . "';return false;\">" . $Language->phrase("MasterDetailListLink") . "</a>";
            }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fijinbpomlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    //    <script>
    //    function selesai(id) {
    //		$.get('/bsd/api/ijinbpom/selesai/'+id, function(data) {
    //			if (data == 1) {
    //				location.reload();
    //			} else {
    ...
    //		$.get('/bsd/api/ijinbpom/belumselesai/'+id, function(data) {
    //			if (data == 1) {
    //				location.reload();
    //			} else {
    //				alert('Gagal menandai belum selesai.');
    //			}
    //		});
    //	}
    //    </script>
