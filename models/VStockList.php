 ...
 * Page class
 */
class VStockList extends VStock
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'v_stock';

    // Page object name
    public $PageObjName = "VStockList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fv_stocklist";
    ...
    public function __construct()
    {
        ...
        // Table object (v_stock)
        if (!isset($GLOBALS["v_stock"]) || get_class($GLOBALS["v_stock"]) == PROJECT_NAMESPACE . "v_stock") {
            $GLOBALS["v_stock"] = &$this;
        }
        ...
        $this->AddUrl = "VStockAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "VStockDelete";
        $this->MultiUpdateUrl = "VStockUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'v_stock');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fv_stocklistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("v_stock"));
                $doc->Text = @$content;
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->idorder_detail->AdvancedSearch->toJson(), ","); // Field idorder_detail
        ...
        $filterList = Concat($filterList, $this->jumlahorder->AdvancedSearch->toJson(), ","); // Field jumlahorder
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fv_stocklistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field idorder_detail
        $this->idorder_detail->AdvancedSearch->SearchValue = @$filter["x_idorder_detail"];
        $this->idorder_detail->AdvancedSearch->SearchOperator = @$filter["z_idorder_detail"];
        $this->idorder_detail->AdvancedSearch->SearchCondition = @$filter["v_idorder_detail"];
        $this->idorder_detail->AdvancedSearch->SearchValue2 = @$filter["y_idorder_detail"];
        $this->idorder_detail->AdvancedSearch->SearchOperator2 = @$filter["w_idorder_detail"];
        $this->idorder_detail->AdvancedSearch->save();
        ...
        // Field jumlahorder
        $this->jumlahorder->AdvancedSearch->SearchValue = @$filter["x_jumlahorder"];
        $this->jumlahorder->AdvancedSearch->SearchOperator = @$filter["z_jumlahorder"];
        $this->jumlahorder->AdvancedSearch->SearchCondition = @$filter["v_jumlahorder"];
        $this->jumlahorder->AdvancedSearch->SearchValue2 = @$filter["y_jumlahorder"];
        $this->jumlahorder->AdvancedSearch->SearchOperator2 = @$filter["w_jumlahorder"];
        $this->jumlahorder->AdvancedSearch->save();
        ...
        // Field kodepo
        $this->kodepo->AdvancedSearch->SearchValue = @$filter["x_kodepo"];
        $this->kodepo->AdvancedSearch->SearchOperator = @$filter["z_kodepo"];
        $this->kodepo->AdvancedSearch->SearchCondition = @$filter["v_kodepo"];
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->kodepo, $arKeywords, $type);
        return $where;
    }
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->idorder_detail->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fv_stocklistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fv_stocklistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fv_stocklist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fv_stocklistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
