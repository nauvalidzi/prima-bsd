 ...
 * Page class
 */
class SuratjalanDetailGrid extends SuratjalanDetail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SuratjalanDetailGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fsuratjalan_detailgrid";
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_idinvoice") && $CurrentForm->hasValue("o_idinvoice") && $this->idinvoice->CurrentValue != $this->idinvoice->OldValue) {
            return false;
        }
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->idsuratjalan->CurrentValue = null;
        $this->idsuratjalan->OldValue = $this->idsuratjalan->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_idinvoice")) {
            $this->idinvoice->setOldValue($CurrentForm->getValue("o_idinvoice"));
        }
    ...
    protected function newRow()
    {
        ...
        $row['idsuratjalan'] = $this->idsuratjalan->CurrentValue;
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
            $masterFilter = $this->sqlMasterFilter_suratjalan();
            if (strval($this->idsuratjalan->CurrentValue) != "") {
                $masterFilter = str_replace("@id@", AdjustSql($this->idsuratjalan->CurrentValue, "DB"), $masterFilter);
            } else {
                $masterFilter = "";
            }
            if ($masterFilter != "") {
                $rsmaster = Container("suratjalan")->loadRs($masterFilter)->fetch(\PDO::FETCH_ASSOC);
                $masterRecordExists = $rsmaster !== false;
                $validMasterKey = true;
                if ($masterRecordExists) {
                    $validMasterKey = $Security->isValidUserID($rsmaster['created_by']);
                } elseif ($this->getCurrentMasterTable() == "suratjalan") {
        ...
        if ($this->getCurrentMasterTable() == "suratjalan") {
            $this->idsuratjalan->CurrentValue = $this->idsuratjalan->getSessionValue();
        }
        $conn = $this->getConnection();
        ...
        if ($this->idsuratjalan->getSessionValue() != "") {
            $rsnew['idsuratjalan'] = $this->idsuratjalan->getSessionValue();
        }
