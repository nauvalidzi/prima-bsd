 ...
 * Page class
 */
class VPenagihanList extends VPenagihan
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'v_penagihan';

    // Page object name
    public $PageObjName = "VPenagihanList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fv_penagihanlist";
    ...
    public function __construct()
    {
        ...
        // Table object (v_penagihan)
        if (!isset($GLOBALS["v_penagihan"]) || get_class($GLOBALS["v_penagihan"]) == PROJECT_NAMESPACE . "v_penagihan") {
            $GLOBALS["v_penagihan"] = &$this;
        }
        ...
        $this->AddUrl = "VPenagihanAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "VPenagihanDelete";
        $this->MultiUpdateUrl = "VPenagihanUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'v_penagihan');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fv_penagihanlistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("v_penagihan"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->tgl_reminder->setVisibility();
        $this->tgl_normalbilling->setVisibility();
        $this->tgl_jatuhtempo->setVisibility();
        $this->tgl_intensbilling->setVisibility();
        $this->tgl_actionplan->setVisibility();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->tgl_reminder); // tgl_reminder
            $this->updateSort($this->tgl_normalbilling); // tgl_normalbilling
            $this->updateSort($this->tgl_jatuhtempo); // tgl_jatuhtempo
            $this->updateSort($this->tgl_intensbilling); // tgl_intensbilling
            $this->updateSort($this->tgl_actionplan); // tgl_actionplan
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->tgl_reminder->setSort("");
                $this->tgl_normalbilling->setSort("");
                $this->tgl_jatuhtempo->setSort("");
                $this->tgl_intensbilling->setSort("");
                $this->tgl_actionplan->setSort("");
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fv_penagihanlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fv_penagihanlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fv_penagihanlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function newRow()
    {
        $row = [];
        $row['tgl_order'] = null;
        $row['tgl_reminder'] = null;
        $row['tgl_normalbilling'] = null;
        $row['tgl_jatuhtempo'] = null;
        $row['tgl_intensbilling'] = null;
        $row['tgl_actionplan'] = null;
        $row['kodeorder'] = null;
    ...
    public function renderRow()
    {
        ...
        if ($this->nominal->FormValue == $this->nominal->CurrentValue && is_numeric(ConvertToFloatString($this->nominal->CurrentValue))) {
            $this->nominal->CurrentValue = ConvertToFloatString($this->nominal->CurrentValue);
        }

        // Convert decimal values if posted back
        if ($this->nilai_po->FormValue == $this->nilai_po->CurrentValue && is_numeric(ConvertToFloatString($this->nilai_po->CurrentValue))) {
            $this->nilai_po->CurrentValue = ConvertToFloatString($this->nilai_po->CurrentValue);
        }

        // Convert decimal values if posted back
        if ($this->pembayaran->FormValue == $this->pembayaran->CurrentValue && is_numeric(ConvertToFloatString($this->pembayaran->CurrentValue))) {
            $this->pembayaran->CurrentValue = ConvertToFloatString($this->pembayaran->CurrentValue);
        }
