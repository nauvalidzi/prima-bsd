 ...
 * Page class
 */
class IjinhakiStatusGrid extends IjinhakiStatus
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "IjinhakiStatusGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fijinhaki_statusgrid";
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_targetmulai") && $CurrentForm->hasValue("o_targetmulai") && $this->targetmulai->CurrentValue != $this->targetmulai->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_tglmulai") && $CurrentForm->hasValue("o_tglmulai") && $this->tglmulai->CurrentValue != $this->tglmulai->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_targetselesai") && $CurrentForm->hasValue("o_targetselesai") && $this->targetselesai->CurrentValue != $this->targetselesai->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_tglselesai") && $CurrentForm->hasValue("o_tglselesai") && $this->tglselesai->CurrentValue != $this->tglselesai->OldValue) {
            return false;
        }
    ...
    public function resetFormError()
    {
        ...
        $this->targetmulai->clearErrorMessage();
        $this->tglmulai->clearErrorMessage();
        $this->targetselesai->clearErrorMessage();
        $this->tglselesai->clearErrorMessage();
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->idijinhaki->CurrentValue = null;
        $this->idijinhaki->OldValue = $this->idijinhaki->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_targetmulai")) {
            $this->targetmulai->setOldValue($CurrentForm->getValue("o_targetmulai"));
        }
        ...
        if ($CurrentForm->hasValue("o_tglmulai")) {
            $this->tglmulai->setOldValue($CurrentForm->getValue("o_tglmulai"));
        }
        ...
        if ($CurrentForm->hasValue("o_targetselesai")) {
            $this->targetselesai->setOldValue($CurrentForm->getValue("o_targetselesai"));
        }
        ...
        if ($CurrentForm->hasValue("o_tglselesai")) {
            $this->tglselesai->setOldValue($CurrentForm->getValue("o_tglselesai"));
        }
    ...
    protected function newRow()
    {
        ...
        $row['idijinhaki'] = $this->idijinhaki->CurrentValue;
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
            $masterFilter = $this->sqlMasterFilter_ijinhaki();
            if (strval($this->idijinhaki->CurrentValue) != "") {
                $masterFilter = str_replace("@id@", AdjustSql($this->idijinhaki->CurrentValue, "DB"), $masterFilter);
            } else {
                $masterFilter = "";
            }
            if ($masterFilter != "") {
                $rsmaster = Container("ijinhaki")->loadRs($masterFilter)->fetch(\PDO::FETCH_ASSOC);
                $masterRecordExists = $rsmaster !== false;
                $validMasterKey = true;
                if ($masterRecordExists) {
                    $validMasterKey = $Security->isValidUserID($rsmaster['created_by']);
                } elseif ($this->getCurrentMasterTable() == "ijinhaki") {
        ...
        if ($this->getCurrentMasterTable() == "ijinhaki") {
            $this->idijinhaki->CurrentValue = $this->idijinhaki->getSessionValue();
        }

        // Check referential integrity for master table 'ijinhaki_status'
        $validMasterRecord = true;
        $masterFilter = $this->sqlMasterFilter_ijinhaki();
        if ($this->idijinhaki->getSessionValue() != "") {
        $masterFilter = str_replace("@id@", AdjustSql($this->idijinhaki->getSessionValue(), "DB"), $masterFilter);
        } else {
            $validMasterRecord = false;
        }
        if ($validMasterRecord) {
        ...
        // idijinhaki
        if ($this->idijinhaki->getSessionValue() != "") {
            $rsnew['idijinhaki'] = $this->idijinhaki->getSessionValue();
        }
