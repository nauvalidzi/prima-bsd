 ...
 * Page class
 */
class StockDeliveryorderView extends StockDeliveryorder
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "StockDeliveryorderView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id") ?? Route("id")) !== null) {
                ...
                $returnUrl = "StockDeliveryorderList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "StockDeliveryorderList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "StockDeliveryorderList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $item = &$option->add("detail_stock_deliveryorder_detail");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("stock_deliveryorder_detail", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("StockDeliveryorderDetailList?" . Config("TABLE_SHOW_MASTER") . "=stock_deliveryorder&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'stock_deliveryorder')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=stock_deliveryorder_detail"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailViewLink")) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        $item->Body = $body;
        $item->Visible = $Security->allowList(CurrentProjectID() . 'stock_deliveryorder_detail');
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("stock_deliveryorder_detail", $detailTblVar)) {
                $detailPageObj = Container("StockDeliveryorderDetailGrid");
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("StockDeliveryorderList"), "", $this->TableVar, true);
        $pageId = "view";
