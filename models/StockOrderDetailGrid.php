 ...
 * Page class
 */
class StockOrderDetailGrid extends StockOrderDetail
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'stock_order_detail';

    // Page object name
    public $PageObjName = "StockOrderDetailGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fstock_order_detailgrid";
    ...
    public function __construct()
    {
        ...
        // Table object (stock_order_detail)
        if (!isset($GLOBALS["stock_order_detail"]) || get_class($GLOBALS["stock_order_detail"]) == PROJECT_NAMESPACE . "stock_order_detail") {
            $GLOBALS["stock_order_detail"] = &$this;
        }

        // Page URL
        $pageUrl = $this->pageUrl();
        $this->AddUrl = "StockOrderDetailAdd";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'stock_order_detail');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("stock_order_detail"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "stock_order") {
            $masterTbl = Container("stock_order");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("StockOrderList"); // Return to master page
                return;
            } else {
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_idproduct") && $CurrentForm->hasValue("o_idproduct") && $this->idproduct->CurrentValue != $this->idproduct->OldValue) {
            return false;
        }
    ...
    public function resetFormError()
    {
        ...
        $this->idproduct->clearErrorMessage();
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_idproduct")) {
            $this->idproduct->setOldValue($CurrentForm->getValue("o_idproduct"));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            if ($validMasterRecord) {
                $rsmaster = Container("stock_order")->loadRs($masterFilter)->fetch();
                $validMasterRecord = $rsmaster !== false;
            }
            if (!$validMasterRecord) {
                $relatedRecordMsg = str_replace("%t", "stock_order", $Language->phrase("RelatedRecordRequired"));
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'stock_order_detail'
        $validMasterRecord = true;
        $masterFilter = $this->sqlMasterFilter_stock_order();
