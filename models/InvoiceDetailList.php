 ...
 * Page class
 */
class InvoiceDetailList extends InvoiceDetail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "InvoiceDetailList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "finvoice_detaillist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "InvoiceDetailDelete";
        $this->MultiUpdateUrl = "InvoiceDetailUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option finvoice_detaillistsrch";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->stockdo); // stockdo
            $this->updateSort($this->jumlahkirim); // jumlahkirim
            $this->updateSort($this->jumlahbonus); // jumlahbonus
            $this->updateSort($this->harga); // harga
            $this->updateSort($this->totalnondiskon); // totalnondiskon
            $this->updateSort($this->diskonpayment); // diskonpayment
            $this->updateSort($this->bbpersen); // bbpersen
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->bonus->setSort("");
                $this->stockdo->setSort("");
                $this->jumlahkirim->setSort("");
                $this->jumlahbonus->setSort("");
                $this->harga->setSort("");
                $this->totalnondiskon->setSort("");
                $this->diskonpayment->setSort("");
                $this->bbpersen->setSort("");
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"finvoice_detaillistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"finvoice_detaillistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.finvoice_detaillist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
