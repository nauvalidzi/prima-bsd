 ...
 * Page class
 */
class InvoiceDetailPreview extends InvoiceDetail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "InvoiceDetailPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "invoice") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=invoice&" . GetForeignKeyUrl("fk_id", $this->idinvoice->QueryStringValue) . "";
        }
        return $url;
    }
        ...
        if ($masterTblVar == "invoice") {
            ...
            if ($x !== false) {
                ...
                $val = $this->unquoteValue($val, "DB");
                 $this->idinvoice->setQueryStringValue($val);
            }
        }
    }
    ...
    public function pageDataRendering(&$header)
    {
        // Example:
        //$header = "your header";
        $readonly = ExecuteScalar("SELECT readonly FROM invoice WHERE id=".$this->idinvoice->CurrentValue);
        if($readonly) {
