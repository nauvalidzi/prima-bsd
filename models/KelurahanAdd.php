 ...
 * Page class
 */
class KelurahanAdd extends Kelurahan
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "KelurahanAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("KelurahanList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "KelurahanList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "KelurahanView") {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->idkecamatan->EditValue === null) { // Lookup from database
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->idkecamatan->Lookup->renderViewRow($rswrk[0]);
                        $this->idkecamatan->EditValue = $this->idkecamatan->displayValue($arwrk);
                    } else {
                        $this->idkecamatan->EditValue = HtmlEncode($this->idkecamatan->CurrentValue);
                    }
                }
            } else {
                $this->idkecamatan->EditValue = null;
            }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // idkecamatan
        $this->idkecamatan->setDbValueDef($rsnew, $this->idkecamatan->CurrentValue, 0, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("KelurahanList"), "", $this->TableVar, true);
