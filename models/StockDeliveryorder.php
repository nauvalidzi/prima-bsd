...
/**
 * Table class for stock_deliveryorder
 */
class StockDeliveryorder extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'stock_deliveryorder';
        $this->TableName = 'stock_deliveryorder';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`stock_deliveryorder`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id
        $this->id = new DbField('stock_deliveryorder', 'stock_deliveryorder', 'x_id', 'id', '`id`', '`id`', 20, 20, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // tanggal
        $this->tanggal = new DbField('stock_deliveryorder', 'stock_deliveryorder', 'x_tanggal', 'tanggal', '`tanggal`', CastDateFieldForLike("`tanggal`", 7, "DB"), 133, 10, 7, false, '`tanggal`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // created_at
        $this->created_at = new DbField('stock_deliveryorder', 'stock_deliveryorder', 'x_created_at', 'created_at', '`created_at`', CastDateFieldForLike("`created_at`", 11, "DB"), 135, 19, 11, false, '`created_at`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "stock_deliveryorder_detail") {
            $detailUrl = Container("stock_deliveryorder_detail")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "StockDeliveryorderList";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`stock_deliveryorder`";
    }
    ...
    {
        // Cascade Update detail table 'stock_deliveryorder_detail'
        $cascadeUpdate = false;
        $rscascade = [];
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("stock_deliveryorder_detail")->loadRs("`pid` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB'))->fetchAll(\PDO::FETCH_ASSOC);
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("stock_deliveryorder_detail")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("stock_deliveryorder_detail")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("stock_deliveryorder_detail")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'stock_deliveryorder_detail'
        $dtlrows = Container("stock_deliveryorder_detail")->loadRs("`pid` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("stock_deliveryorder_detail")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("stock_deliveryorder_detail")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("stock_deliveryorder_detail")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("StockDeliveryorderList");
    }
    ...
    {
        global $Language;
        if ($pageName == "StockDeliveryorderView") {
            return $Language->phrase("View");
        } elseif ($pageName == "StockDeliveryorderEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "StockDeliveryorderAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "StockDeliveryorderView";
            case Config("API_ADD_ACTION"):
                return "StockDeliveryorderAdd";
            case Config("API_EDIT_ACTION"):
                return "StockDeliveryorderEdit";
            case Config("API_DELETE_ACTION"):
                return "StockDeliveryorderDelete";
            case Config("API_LIST_ACTION"):
                return "StockDeliveryorderList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("StockDeliveryorderView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("StockDeliveryorderView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "StockDeliveryorderAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "StockDeliveryorderAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("StockDeliveryorderEdit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("StockDeliveryorderEdit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("StockDeliveryorderAdd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("StockDeliveryorderAdd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("StockDeliveryorderDelete", $this->getUrlParm());
    }
