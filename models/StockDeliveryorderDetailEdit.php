 ...
 * Page class
 */
class StockDeliveryorderDetailEdit extends StockDeliveryorderDetail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "StockDeliveryorderDetailEdit";
    ...
    public function run()
    {
        ...
        $this->idstock_order_detail->setVisibility();
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'idstock_order_detail' first before field var 'x_idstock_order_detail'
        $val = $CurrentForm->hasValue("idstock_order_detail") ? $CurrentForm->getValue("idstock_order_detail") : $CurrentForm->getValue("x_idstock_order_detail");
        if (!$this->idstock_order_detail->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->idstock_order_detail->Visible = false; // Disable update for API request
            } else {
                $this->idstock_order_detail->setFormValue($val);
            }
        }
        ...
        // Check field name 'jumlah_kirim' first before field var 'x_jumlah_kirim'
        $val = $CurrentForm->hasValue("jumlah_kirim") ? $CurrentForm->getValue("jumlah_kirim") : $CurrentForm->getValue("x_jumlah_kirim");
        if (!$this->jumlah_kirim->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->jumlah_kirim->Visible = false; // Disable update for API request
            } else {
                $this->jumlah_kirim->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->idstock_order_detail->CurrentValue = $this->idstock_order_detail->FormValue;
        $this->totalorder->CurrentValue = $this->totalorder->FormValue;
        $this->jumlah_kirim->CurrentValue = $this->jumlah_kirim->FormValue;
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->idstock_order_detail->setDbValue($row['idstock_order_detail']);
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // idstock_order_detail
            $this->idstock_order_detail->ViewValue = $this->idstock_order_detail->CurrentValue;
            $this->idstock_order_detail->ViewValue = FormatNumber($this->idstock_order_detail->ViewValue, 0, -2, -2, -2);
            $this->idstock_order_detail->ViewCustomAttributes = "";
            ...
            // idstock_order_detail
            $this->idstock_order_detail->LinkCustomAttributes = "";
            $this->idstock_order_detail->HrefValue = "";
            $this->idstock_order_detail->TooltipValue = "";
            ...
            // idstock_order_detail
            $this->idstock_order_detail->EditAttrs["class"] = "form-control";
            $this->idstock_order_detail->EditCustomAttributes = "";
            $this->idstock_order_detail->EditValue = HtmlEncode($this->idstock_order_detail->CurrentValue);
            $this->idstock_order_detail->PlaceHolder = RemoveHtml($this->idstock_order_detail->caption());
            ...
            $this->jumlah_kirim->EditValue = HtmlEncode($this->jumlah_kirim->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->idstock_order_detail->Required) {
            if (!$this->idstock_order_detail->IsDetailKey && EmptyValue($this->idstock_order_detail->FormValue)) {
                $this->idstock_order_detail->addErrorMessage(str_replace("%s", $this->idstock_order_detail->caption(), $this->idstock_order_detail->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->idstock_order_detail->FormValue)) {
            $this->idstock_order_detail->addErrorMessage($this->idstock_order_detail->getErrorMessage(false));
        }
        ...
        if ($this->jumlah_kirim->Required) {
            if (!$this->jumlah_kirim->IsDetailKey && EmptyValue($this->jumlah_kirim->FormValue)) {
                $this->jumlah_kirim->addErrorMessage(str_replace("%s", $this->jumlah_kirim->caption(), $this->jumlah_kirim->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->jumlah_kirim->FormValue)) {
            $this->jumlah_kirim->addErrorMessage($this->jumlah_kirim->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // idstock_order_detail
            $this->idstock_order_detail->setDbValueDef($rsnew, $this->idstock_order_detail->CurrentValue, 0, $this->idstock_order_detail->ReadOnly);

            // totalorder
            $this->totalorder->setDbValueDef($rsnew, $this->totalorder->CurrentValue, 0, $this->totalorder->ReadOnly);

            // jumlah_kirim
            $this->jumlah_kirim->setDbValueDef($rsnew, $this->jumlah_kirim->CurrentValue, 0, $this->jumlah_kirim->ReadOnly);
            ...
            // Check referential integrity for master table 'stock_deliveryorder'
