 ...
 * Page class
 */
class VPiutangList extends VPiutang
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'v_piutang';

    // Page object name
    public $PageObjName = "VPiutangList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fv_piutanglist";
    ...
    public function __construct()
    {
        ...
        // Table object (v_piutang)
        if (!isset($GLOBALS["v_piutang"]) || get_class($GLOBALS["v_piutang"]) == PROJECT_NAMESPACE . "v_piutang") {
            $GLOBALS["v_piutang"] = &$this;
        }
        ...
        $this->AddUrl = "VPiutangAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "VPiutangDelete";
        $this->MultiUpdateUrl = "VPiutangUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'v_piutang');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fv_piutanglistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("v_piutang"));
                $doc->Text = @$content;
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fv_piutanglistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field totalpiutang
        $this->totalpiutang->AdvancedSearch->SearchValue = @$filter["x_totalpiutang"];
        $this->totalpiutang->AdvancedSearch->SearchOperator = @$filter["z_totalpiutang"];
        $this->totalpiutang->AdvancedSearch->SearchCondition = @$filter["v_totalpiutang"];
        $this->totalpiutang->AdvancedSearch->SearchValue2 = @$filter["y_totalpiutang"];
        $this->totalpiutang->AdvancedSearch->SearchOperator2 = @$filter["w_totalpiutang"];
        $this->totalpiutang->AdvancedSearch->save();
    ...
    protected function loadSortOrder()
    {
        ...
        if ($orderBy == "") {
            $this->DefaultSort = "`totaltagihan` DESC";
            if ($this->getSqlOrderBy() != "") {
                $useDefaultSort = true;
                if ($this->totaltagihan->getSort() != "") {
                    $useDefaultSort = false;
                }
                if ($useDefaultSort) {
                    $this->totaltagihan->setSort("DESC");
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_v_piutang_detail"
        $item = &$this->ListOptions->add("detail_v_piutang_detail");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'v_piutang_detail') && !$this->ShowMultipleDetails;
        $item->OnLeft = false;
    ...
    public function renderListOptions()
    {
        ...
        // "detail_v_piutang_detail"
        $opt = $this->ListOptions["detail_v_piutang_detail"];
        if ($Security->allowList(CurrentProjectID() . 'v_piutang_detail') && $this->showOptionLink()) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("v_piutang_detail", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("VPiutangDetailList?" . Config("TABLE_SHOW_MASTER") . "=v_piutang&" . GetForeignKeyUrl("fk_idcustomer", $this->idcustomer->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("VPiutangDetailGrid");
            if ($links != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fv_piutanglistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fv_piutanglistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fv_piutanglist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function renderListOptionsExt()
    {
        ...
        // Column "detail_v_piutang_detail"
        if ($this->DetailPages && $this->DetailPages["v_piutang_detail"] && $this->DetailPages["v_piutang_detail"]->Visible) {
            $link = "";
            $option = $this->ListOptions["detail_v_piutang_detail"];
            $url = "VPiutangDetailPreview?t=v_piutang&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"v_piutang_detail\" data-url=\"" . $url . "\">";
            if ($Security->allowList(CurrentProjectID() . 'v_piutang')) {
                $label = $Language->TablePhrase("v_piutang_detail", "TblCaption");
                $link = "<li class=\"nav-item\"><a href=\"#\" class=\"nav-link\" data-toggle=\"tab\" data-table=\"v_piutang_detail\" data-url=\"" . $url . "\">" . $label . "</a></li>";
                $links .= $link;
                $detaillnk = JsEncodeAttribute("VPiutangDetailList?" . Config("TABLE_SHOW_MASTER") . "=v_piutang&" . GetForeignKeyUrl("fk_idcustomer", $this->idcustomer->CurrentValue) . "");
                $btngrp .= "<a href=\"#\" class=\"mr-2\" title=\"" . $Language->TablePhrase("v_piutang_detail", "TblCaption") . "\" onclick=\"window.location='" . $detaillnk . "';return false;\">" . $Language->phrase("MasterDetailListLink") . "</a>";
            }
            $detailPageObj = Container("VPiutangDetailGrid");
            if ($detailPageObj->DetailView && $Security->canView() && $this->showOptionLink("view") && $Security->allowView(CurrentProjectID() . 'v_piutang')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=v_piutang_detail");
    ...
    public function renderRow()
    {
        ...
        if ($this->totaltagihan->FormValue == $this->totaltagihan->CurrentValue && is_numeric(ConvertToFloatString($this->totaltagihan->CurrentValue))) {
            $this->totaltagihan->CurrentValue = ConvertToFloatString($this->totaltagihan->CurrentValue);
        }

        // Convert decimal values if posted back
        if ($this->totalpiutang->FormValue == $this->totalpiutang->CurrentValue && is_numeric(ConvertToFloatString($this->totalpiutang->CurrentValue))) {
            $this->totalpiutang->CurrentValue = ConvertToFloatString($this->totalpiutang->CurrentValue);
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fv_piutanglistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
