 ...
 * Page class
 */
class DeliveryorderDetailPreview extends DeliveryorderDetail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DeliveryorderDetailPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "deliveryorder") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=deliveryorder&" . GetForeignKeyUrl("fk_id", $this->iddeliveryorder->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeysFromFilter($f)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "deliveryorder") {
            $find = "`iddeliveryorder`=";
            $x = strpos($f, $find);
            if ($x !== false) {
                $x += strlen($find);
                $val = substr($f, $x);
                $val = $this->unquoteValue($val, "DB");
                 $this->iddeliveryorder->setQueryStringValue($val);
            }
        }
    }
    ...
    public function pageDataRendering(&$header)
    {
        // Example:
        //$header = "your header";
        $readonly = ExecuteScalar("SELECT readonly FROM deliveryorder WHERE id=".$this->iddeliveryorder->CurrentValue);
        if($readonly) {
