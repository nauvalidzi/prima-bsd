 ...
 */
class VOrderdetail extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // id
        $this->id = new DbField('v_orderdetail', 'v_orderdetail', 'x_id', 'id', '`id`', '`id`', 20, 20, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'NO');
        ...
        // nama
        $this->nama = new DbField('v_orderdetail', 'v_orderdetail', 'x_nama', 'nama', '`nama`', '`nama`', 200, 255, -1, false, '`nama`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // aktif
        $this->aktif = new DbField('v_orderdetail', 'v_orderdetail', 'x_aktif', 'aktif', '`aktif`', '`aktif`', 16, 1, -1, false, '`aktif`', false, false, false, 'FORMATTED TEXT', 'CHECKBOX');
        $this->aktif->Nullable = false; // NOT NULL field
        $this->aktif->Sortable = true; // Allow sort
        $this->aktif->DataType = DATATYPE_BOOLEAN;
        switch ($CurrentLanguage) {
            case "en":
                $this->aktif->Lookup = new Lookup('aktif', 'v_orderdetail', false, '', ["","","",""], [], [], [], [], [], [], '', '');
                break;
            default:
                $this->aktif->Lookup = new Lookup('aktif', 'v_orderdetail', false, '', ["","","",""], [], [], [], [], [], [], '', '');
                break;
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("VOrderdetailList");
    }
    ...
    {
        global $Language;
        if ($pageName == "VOrderdetailView") {
            return $Language->phrase("View");
        } elseif ($pageName == "VOrderdetailEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "VOrderdetailAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "VOrderdetailView";
            case Config("API_ADD_ACTION"):
                return "VOrderdetailAdd";
            case Config("API_EDIT_ACTION"):
                return "VOrderdetailEdit";
            case Config("API_DELETE_ACTION"):
                return "VOrderdetailDelete";
            case Config("API_LIST_ACTION"):
                return "VOrderdetailList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("VOrderdetailView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("VOrderdetailView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "VOrderdetailAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "VOrderdetailAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("VOrderdetailEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("VOrderdetailAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("VOrderdetailDelete", $this->getUrlParm());
    }
