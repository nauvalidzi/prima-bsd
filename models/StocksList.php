 ...
 * Page class
 */
class StocksList extends Stocks
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "StocksList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fstockslist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "StocksAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "StocksDelete";
        $this->MultiUpdateUrl = "StocksUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fstockslistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->prop_id->AdvancedSearch->toJson(), ","); // Field prop_id
        ...
        $filterList = Concat($filterList, $this->stok_masuk->AdvancedSearch->toJson(), ","); // Field stok_masuk
        $filterList = Concat($filterList, $this->stok_keluar->AdvancedSearch->toJson(), ","); // Field stok_keluar
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fstockslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field prop_id
        $this->prop_id->AdvancedSearch->SearchValue = @$filter["x_prop_id"];
        $this->prop_id->AdvancedSearch->SearchOperator = @$filter["z_prop_id"];
        $this->prop_id->AdvancedSearch->SearchCondition = @$filter["v_prop_id"];
        $this->prop_id->AdvancedSearch->SearchValue2 = @$filter["y_prop_id"];
        $this->prop_id->AdvancedSearch->SearchOperator2 = @$filter["w_prop_id"];
        $this->prop_id->AdvancedSearch->save();
        ...
        // Field stok_masuk
        $this->stok_masuk->AdvancedSearch->SearchValue = @$filter["x_stok_masuk"];
        $this->stok_masuk->AdvancedSearch->SearchOperator = @$filter["z_stok_masuk"];
        $this->stok_masuk->AdvancedSearch->SearchCondition = @$filter["v_stok_masuk"];
        $this->stok_masuk->AdvancedSearch->SearchValue2 = @$filter["y_stok_masuk"];
        $this->stok_masuk->AdvancedSearch->SearchOperator2 = @$filter["w_stok_masuk"];
        $this->stok_masuk->AdvancedSearch->save();

        // Field stok_keluar
        $this->stok_keluar->AdvancedSearch->SearchValue = @$filter["x_stok_keluar"];
        $this->stok_keluar->AdvancedSearch->SearchOperator = @$filter["z_stok_keluar"];
        $this->stok_keluar->AdvancedSearch->SearchCondition = @$filter["v_stok_keluar"];
        $this->stok_keluar->AdvancedSearch->SearchValue2 = @$filter["y_stok_keluar"];
        $this->stok_keluar->AdvancedSearch->SearchOperator2 = @$filter["w_stok_keluar"];
        $this->stok_keluar->AdvancedSearch->save();
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->prop_code, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->prop_id); // prop_id
            $this->updateSort($this->prop_code); // prop_code
            $this->updateSort($this->idproduct); // idproduct
            $this->updateSort($this->stok_masuk); // stok_masuk
            $this->updateSort($this->stok_keluar); // stok_keluar
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->setSessionOrderBy($orderBy);
                $this->id->setSort("");
                $this->prop_id->setSort("");
                $this->prop_code->setSort("");
                $this->idproduct->setSort("");
                $this->stok_masuk->setSort("");
                $this->stok_keluar->setSort("");
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fstockslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fstockslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fstockslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fstockslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
