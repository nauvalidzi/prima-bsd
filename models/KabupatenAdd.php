 ...
 * Page class
 */
class KabupatenAdd extends Kabupaten
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "KabupatenAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("KabupatenList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "KabupatenList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "KabupatenView") {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->idprovinsi->EditValue === null) { // Lookup from database
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->idprovinsi->Lookup->renderViewRow($rswrk[0]);
                        $this->idprovinsi->EditValue = $this->idprovinsi->displayValue($arwrk);
                    } else {
                        $this->idprovinsi->EditValue = HtmlEncode($this->idprovinsi->CurrentValue);
                    }
                }
            } else {
                $this->idprovinsi->EditValue = null;
            }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // idprovinsi
        $this->idprovinsi->setDbValueDef($rsnew, $this->idprovinsi->CurrentValue, 0, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("KabupatenList"), "", $this->TableVar, true);
