 ...
 * Page class
 */
class IjinbpomStatusList extends IjinbpomStatus
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "IjinbpomStatusList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fijinbpom_statuslist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "IjinbpomStatusAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "IjinbpomStatusDelete";
        $this->MultiUpdateUrl = "IjinbpomStatusUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fijinbpom_statuslistsrch";
    ...
    public function run()
    {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
                if ($this->getCurrentMasterTable() == "ijinbpom") {
                    $this->DbMasterFilter = $this->addMasterUserIDFilter($this->DbMasterFilter, "ijinbpom"); // Add master User ID filter
                }
        }
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "ijinbpom") {
            $masterTbl = Container("ijinbpom");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("IjinbpomList"); // Return to master page
                return;
            } else {
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->idijinbpom->AdvancedSearch->toJson(), ","); // Field idijinbpom
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fijinbpom_statuslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field idijinbpom
        $this->idijinbpom->AdvancedSearch->SearchValue = @$filter["x_idijinbpom"];
        $this->idijinbpom->AdvancedSearch->SearchOperator = @$filter["z_idijinbpom"];
        $this->idijinbpom->AdvancedSearch->SearchCondition = @$filter["v_idijinbpom"];
        $this->idijinbpom->AdvancedSearch->SearchValue2 = @$filter["y_idijinbpom"];
        $this->idijinbpom->AdvancedSearch->SearchOperator2 = @$filter["w_idijinbpom"];
        $this->idijinbpom->AdvancedSearch->save();
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fijinbpom_statuslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fijinbpom_statuslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fijinbpom_statuslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fijinbpom_statuslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
