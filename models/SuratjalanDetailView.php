 ...
 * Page class
 */
class SuratjalanDetailView extends SuratjalanDetail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SuratjalanDetailView";
    ...
    public function run()
    {
        ...
        $this->idsuratjalan->setVisibility();
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id") ?? Route("id")) !== null) {
                ...
                $returnUrl = "SuratjalanDetailList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "SuratjalanDetailList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "SuratjalanDetailList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("InvoiceList?" . Config("TABLE_SHOW_MASTER") . "=suratjalan_detail&" . GetForeignKeyUrl("fk_idinvoice", $this->idinvoice->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("InvoiceGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'suratjalan_detail')) {
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'suratjalan_detail')) {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("SuratjalanDetailList"), "", $this->TableVar, true);
        $pageId = "view";
