 ...
 * Page class
 */
class VPiutangSearch extends VPiutang
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VPiutangSearch";
    ...
    protected function buildAdvancedSearch()
    {
        ...
        $this->buildSearchUrl($srchUrl, $this->idcustomer); // idcustomer
        $this->buildSearchUrl($srchUrl, $this->totaltagihan); // totaltagihan
        $this->buildSearchUrl($srchUrl, $this->totalpiutang); // totalpiutang
        if ($srchUrl != "") {
            $srchUrl .= "&";
        }
        $srchUrl .= "cmd=search";
    ...
    protected function loadSearchValues()
    {
        ...
        if ($this->idcustomer->AdvancedSearch->post()) {
            $hasValue = true;
        }
        if ($this->totaltagihan->AdvancedSearch->post()) {
            $hasValue = true;
        }
        if ($this->totalpiutang->AdvancedSearch->post()) {
            $hasValue = true;
        }
        return $hasValue;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (!$Security->isAdmin() && $Security->isLoggedIn() && !$this->userIDAllow("search")) { // Non system admin
                if (trim(strval($this->idpegawai->AdvancedSearch->SearchValue)) == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`id`" . SearchString("=", $this->idpegawai->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
                AddFilter($filterWrk, Container("pegawai")->addUserIDFilter());
                $sqlWrk = $this->idpegawai->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $arwrk = $rswrk;
                $this->idpegawai->EditValue = $arwrk;
            } else {
                $this->idpegawai->EditValue = HtmlEncode($this->idpegawai->AdvancedSearch->SearchValue);
            ...
            $this->idcustomer->EditValue = HtmlEncode($this->idcustomer->AdvancedSearch->SearchValue);
            $curVal = trim(strval($this->idcustomer->AdvancedSearch->SearchValue));
            if ($curVal != "") {
            ...
            $this->totaltagihan->EditValue = HtmlEncode($this->totaltagihan->AdvancedSearch->SearchValue);
            ...
            $this->totalpiutang->EditValue = HtmlEncode($this->totalpiutang->AdvancedSearch->SearchValue);
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckInteger($this->idpegawai->AdvancedSearch->SearchValue)) {
            $this->idpegawai->addErrorMessage($this->idpegawai->getErrorMessage(false));
        }
        if (!CheckInteger($this->idcustomer->AdvancedSearch->SearchValue)) {
            $this->idcustomer->addErrorMessage($this->idcustomer->getErrorMessage(false));
        }
        if (!CheckNumber($this->totaltagihan->AdvancedSearch->SearchValue)) {
            $this->totaltagihan->addErrorMessage($this->totaltagihan->getErrorMessage(false));
        }
        if (!CheckNumber($this->totalpiutang->AdvancedSearch->SearchValue)) {
            $this->totalpiutang->addErrorMessage($this->totalpiutang->getErrorMessage(false));
        }
    ...
    public function loadAdvancedSearch()
    {
        ...
        $this->idcustomer->AdvancedSearch->load();
        $this->totaltagihan->AdvancedSearch->load();
        $this->totalpiutang->AdvancedSearch->load();
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("VPiutangList"), "", $this->TableVar, true);
        $pageId = "search";
