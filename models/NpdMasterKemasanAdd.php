 ...
 * Page class
 */
class NpdMasterKemasanAdd extends NpdMasterKemasan
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "NpdMasterKemasanAdd";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page
                    ...
                    if ($pageName == "NpdMasterKemasanView") {
                        $row["view"] = "1";
                    }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("NpdMasterKemasanList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "NpdMasterKemasanList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "NpdMasterKemasanView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->subjenis->CurrentValue = null;
        $this->subjenis->OldValue = $this->subjenis->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'subjenis' first before field var 'x_subjenis'
        $val = $CurrentForm->hasValue("subjenis") ? $CurrentForm->getValue("subjenis") : $CurrentForm->getValue("x_subjenis");
        if (!$this->subjenis->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->subjenis->Visible = false; // Disable update for API request
            } else {
                $this->subjenis->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->subjenis->CurrentValue = $this->subjenis->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['subjenis'] = $this->subjenis->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->subjenis->EditValue = HtmlEncode($this->subjenis->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->subjenis->Required) {
            if (!$this->subjenis->IsDetailKey && EmptyValue($this->subjenis->FormValue)) {
                $this->subjenis->addErrorMessage(str_replace("%s", $this->subjenis->caption(), $this->subjenis->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // subjenis
        $this->subjenis->setDbValueDef($rsnew, $this->subjenis->CurrentValue, null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("NpdMasterKemasanList"), "", $this->TableVar, true);
