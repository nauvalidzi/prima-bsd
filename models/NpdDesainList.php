 ...
 * Page class
 */
class NpdDesainList extends NpdDesain
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "NpdDesainList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fnpd_desainlist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "NpdDesainDelete";
        $this->MultiUpdateUrl = "NpdDesainUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fnpd_desainlistsrch";
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fnpd_desainlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->nama_produk->AdvancedSearch->SearchOperator = @$filter["z_nama_produk"];
        $this->nama_produk->AdvancedSearch->SearchCondition = @$filter["v_nama_produk"];
        $this->nama_produk->AdvancedSearch->SearchValue2 = @$filter["y_nama_produk"];
        $this->nama_produk->AdvancedSearch->SearchOperator2 = @$filter["w_nama_produk"];
        $this->nama_produk->AdvancedSearch->save();

        // Field klaim_bahan
        $this->klaim_bahan->AdvancedSearch->SearchValue = @$filter["x_klaim_bahan"];
        $this->klaim_bahan->AdvancedSearch->SearchOperator = @$filter["z_klaim_bahan"];
        $this->klaim_bahan->AdvancedSearch->SearchCondition = @$filter["v_klaim_bahan"];
        $this->klaim_bahan->AdvancedSearch->SearchValue2 = @$filter["y_klaim_bahan"];
        $this->klaim_bahan->AdvancedSearch->SearchOperator2 = @$filter["w_klaim_bahan"];
        $this->klaim_bahan->AdvancedSearch->save();

        // Field campaign_produk
        $this->campaign_produk->AdvancedSearch->SearchValue = @$filter["x_campaign_produk"];
        $this->campaign_produk->AdvancedSearch->SearchOperator = @$filter["z_campaign_produk"];
        $this->campaign_produk->AdvancedSearch->SearchCondition = @$filter["v_campaign_produk"];
        $this->campaign_produk->AdvancedSearch->SearchValue2 = @$filter["y_campaign_produk"];
        ...
        $this->no_notifikasi->AdvancedSearch->SearchOperator = @$filter["z_no_notifikasi"];
        $this->no_notifikasi->AdvancedSearch->SearchCondition = @$filter["v_no_notifikasi"];
        $this->no_notifikasi->AdvancedSearch->SearchValue2 = @$filter["y_no_notifikasi"];
        $this->no_notifikasi->AdvancedSearch->SearchOperator2 = @$filter["w_no_notifikasi"];
        $this->no_notifikasi->AdvancedSearch->save();

        // Field jenis_kemasan
        $this->jenis_kemasan->AdvancedSearch->SearchValue = @$filter["x_jenis_kemasan"];
        $this->jenis_kemasan->AdvancedSearch->SearchOperator = @$filter["z_jenis_kemasan"];
        $this->jenis_kemasan->AdvancedSearch->SearchCondition = @$filter["v_jenis_kemasan"];
        $this->jenis_kemasan->AdvancedSearch->SearchValue2 = @$filter["y_jenis_kemasan"];
        $this->jenis_kemasan->AdvancedSearch->SearchOperator2 = @$filter["w_jenis_kemasan"];
        $this->jenis_kemasan->AdvancedSearch->save();

        // Field posisi_label
        $this->posisi_label->AdvancedSearch->SearchValue = @$filter["x_posisi_label"];
        $this->posisi_label->AdvancedSearch->SearchOperator = @$filter["z_posisi_label"];
        $this->posisi_label->AdvancedSearch->SearchCondition = @$filter["v_posisi_label"];
        $this->posisi_label->AdvancedSearch->SearchValue2 = @$filter["y_posisi_label"];
        $this->posisi_label->AdvancedSearch->SearchOperator2 = @$filter["w_posisi_label"];
        $this->posisi_label->AdvancedSearch->save();

        // Field bahan_label
        $this->bahan_label->AdvancedSearch->SearchValue = @$filter["x_bahan_label"];
        $this->bahan_label->AdvancedSearch->SearchOperator = @$filter["z_bahan_label"];
        $this->bahan_label->AdvancedSearch->SearchCondition = @$filter["v_bahan_label"];
        $this->bahan_label->AdvancedSearch->SearchValue2 = @$filter["y_bahan_label"];
        $this->bahan_label->AdvancedSearch->SearchOperator2 = @$filter["w_bahan_label"];
        $this->bahan_label->AdvancedSearch->save();

        // Field draft_layout
        $this->draft_layout->AdvancedSearch->SearchValue = @$filter["x_draft_layout"];
        $this->draft_layout->AdvancedSearch->SearchOperator = @$filter["z_draft_layout"];
        $this->draft_layout->AdvancedSearch->SearchCondition = @$filter["v_draft_layout"];
        $this->draft_layout->AdvancedSearch->SearchValue2 = @$filter["y_draft_layout"];
        $this->draft_layout->AdvancedSearch->SearchOperator2 = @$filter["w_draft_layout"];
        $this->draft_layout->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->bahan_label, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->draft_layout, $arKeywords, $type);
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fnpd_desainlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fnpd_desainlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fnpd_desainlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fnpd_desainlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
