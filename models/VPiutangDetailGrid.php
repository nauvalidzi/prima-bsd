 ...
 * Page class
 */
class VPiutangDetailGrid extends VPiutangDetail
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'v_piutang_detail';

    // Page object name
    public $PageObjName = "VPiutangDetailGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fv_piutang_detailgrid";
    ...
    public function __construct()
    {
        ...
        // Table object (v_piutang_detail)
        if (!isset($GLOBALS["v_piutang_detail"]) || get_class($GLOBALS["v_piutang_detail"]) == PROJECT_NAMESPACE . "v_piutang_detail") {
            $GLOBALS["v_piutang_detail"] = &$this;
        }

        // Page URL
        $pageUrl = $this->pageUrl();
        $this->AddUrl = "VPiutangDetailAdd";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'v_piutang_detail');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("v_piutang_detail"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
                if ($this->getCurrentMasterTable() == "v_piutang") {
                    $this->DbMasterFilter = $this->addMasterUserIDFilter($this->DbMasterFilter, "v_piutang"); // Add master User ID filter
                }
        }
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "v_piutang") {
            $masterTbl = Container("v_piutang");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("VPiutangList"); // Return to master page
                return;
            } else {
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->idinvoice->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_tglinvoice") && $CurrentForm->hasValue("o_tglinvoice") && $this->tglinvoice->CurrentValue != $this->tglinvoice->OldValue) {
            return false;
        }
        ...
        if ($CurrentForm->hasValue("x_jatuhtempo") && $CurrentForm->hasValue("o_jatuhtempo") && $this->jatuhtempo->CurrentValue != $this->jatuhtempo->OldValue) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->jatuhtempo->clearErrorMessage();
    }
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->sisabayar->OldValue = $this->sisabayar->CurrentValue;
        $this->totaltagihan->CurrentValue = 0;
        $this->totaltagihan->OldValue = $this->totaltagihan->CurrentValue;
        $this->jatuhtempo->CurrentValue = null;
        $this->jatuhtempo->OldValue = $this->jatuhtempo->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_tglinvoice")) {
            $this->tglinvoice->setOldValue($CurrentForm->getValue("o_tglinvoice"));
        }

        // Check field name 'sisabayar' first before field var 'x_sisabayar'
        $val = $CurrentForm->hasValue("sisabayar") ? $CurrentForm->getValue("sisabayar") : $CurrentForm->getValue("x_sisabayar");
        if (!$this->sisabayar->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->sisabayar->Visible = false; // Disable update for API request
            } else {
                $this->sisabayar->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_sisabayar")) {
        ...
        if ($CurrentForm->hasValue("o_totaltagihan")) {
            $this->totaltagihan->setOldValue($CurrentForm->getValue("o_totaltagihan"));
        }

        // Check field name 'jatuhtempo' first before field var 'x_jatuhtempo'
        $val = $CurrentForm->hasValue("jatuhtempo") ? $CurrentForm->getValue("jatuhtempo") : $CurrentForm->getValue("x_jatuhtempo");
        if (!$this->jatuhtempo->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->jatuhtempo->Visible = false; // Disable update for API request
            } else {
                $this->jatuhtempo->setFormValue($val);
            }
            $this->jatuhtempo->CurrentValue = UnFormatDateTime($this->jatuhtempo->CurrentValue, 0);
        }
        if ($CurrentForm->hasValue("o_jatuhtempo")) {
            $this->jatuhtempo->setOldValue($CurrentForm->getValue("o_jatuhtempo"));
        }
    ...
    protected function newRow()
    {
        ...
        $row['jatuhtempo'] = $this->jatuhtempo->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->jatuhtempo->EditValue = HtmlEncode(FormatDateTime($this->jatuhtempo->CurrentValue, 8));
    ...
    protected function validateForm()
    {
        ...
        if ($this->sisabayar->Required) {
            if (!$this->sisabayar->IsDetailKey && EmptyValue($this->sisabayar->FormValue)) {
                $this->sisabayar->addErrorMessage(str_replace("%s", $this->sisabayar->caption(), $this->sisabayar->RequiredErrorMessage));
            }
        }
        ...
        if ($this->jatuhtempo->Required) {
            if (!$this->jatuhtempo->IsDetailKey && EmptyValue($this->jatuhtempo->FormValue)) {
                $this->jatuhtempo->addErrorMessage(str_replace("%s", $this->jatuhtempo->caption(), $this->jatuhtempo->RequiredErrorMessage));
            }
        }
        if (!CheckDate($this->jatuhtempo->FormValue)) {
            $this->jatuhtempo->addErrorMessage($this->jatuhtempo->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // jatuhtempo
            $this->jatuhtempo->setDbValueDef($rsnew, UnFormatDateTime($this->jatuhtempo->CurrentValue, 0), null, $this->jatuhtempo->ReadOnly);
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
            $masterFilter = $this->sqlMasterFilter_v_piutang();
            ...
            if ($masterFilter != "") {
                $rsmaster = Container("v_piutang")->loadRs($masterFilter)->fetch(\PDO::FETCH_ASSOC);
                $masterRecordExists = $rsmaster !== false;
                $validMasterKey = true;
                if ($masterRecordExists) {
                    $validMasterKey = $Security->isValidUserID($rsmaster['idpegawai']);
                } elseif ($this->getCurrentMasterTable() == "v_piutang") {
