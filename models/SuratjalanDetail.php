...
/**
 * Table class for suratjalan_detail
 */
class SuratjalanDetail extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'suratjalan_detail';
        $this->TableName = 'suratjalan_detail';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`suratjalan_detail`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id
        $this->id = new DbField('suratjalan_detail', 'suratjalan_detail', 'x_id', 'id', '`id`', '`id`', 20, 20, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'NO');
        ...
        // created_at
        $this->created_at = new DbField('suratjalan_detail', 'suratjalan_detail', 'x_created_at', 'created_at', '`created_at`', CastDateFieldForLike("`created_at`", 0, "DB"), 135, 19, 0, false, '`created_at`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // created_by
        $this->created_by = new DbField('suratjalan_detail', 'suratjalan_detail', 'x_created_by', 'created_by', '`created_by`', '`created_by`', 3, 11, -1, false, '`created_by`', false, false, false, 'FORMATTED TEXT', 'HIDDEN');
    ...
    public function getMasterFilter()
    {
        ...
        if ($this->getCurrentMasterTable() == "suratjalan") {
            if ($this->idsuratjalan->getSessionValue() != "") {
                $masterFilter .= "" . GetForeignKeySql("`id`", $this->idsuratjalan->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilter()
    {
        ...
        if ($this->getCurrentMasterTable() == "suratjalan") {
            if ($this->idsuratjalan->getSessionValue() != "") {
                $detailFilter .= "" . GetForeignKeySql("`idsuratjalan`", $this->idsuratjalan->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
    ...
    // Master filter
    public function sqlMasterFilter_suratjalan()
    {
        return "`id`=@id@";
    }
    // Detail filter
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`suratjalan_detail`";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->idsuratjalan->DbValue = $row['idsuratjalan'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("SuratjalanDetailList");
    }
    ...
    {
        global $Language;
        if ($pageName == "SuratjalanDetailView") {
            return $Language->phrase("View");
        } elseif ($pageName == "SuratjalanDetailEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "SuratjalanDetailAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "SuratjalanDetailView";
            case Config("API_ADD_ACTION"):
                return "SuratjalanDetailAdd";
            case Config("API_EDIT_ACTION"):
                return "SuratjalanDetailEdit";
            case Config("API_DELETE_ACTION"):
                return "SuratjalanDetailDelete";
            case Config("API_LIST_ACTION"):
                return "SuratjalanDetailList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("SuratjalanDetailView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("SuratjalanDetailView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "SuratjalanDetailAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "SuratjalanDetailAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("SuratjalanDetailDelete", $this->getUrlParm());
    }

    // Add master url
    public function addMasterUrl($url)
    {
        if ($this->getCurrentMasterTable() == "suratjalan" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->idsuratjalan->CurrentValue ?? $this->idsuratjalan->getSessionValue());
        }
        return $url;
    }
...
SORTHTML;
    ...
    public function renderListRow()
    {
        ...
        // idsuratjalan
        $this->idsuratjalan->LinkCustomAttributes = "";
        $this->idsuratjalan->HrefValue = "";
        $this->idsuratjalan->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        $this->idsuratjalan->EditCustomAttributes = "";
        if ($this->idsuratjalan->getSessionValue() != "") {
            $this->idsuratjalan->CurrentValue = GetForeignKeyValue($this->idsuratjalan->getSessionValue());
            $this->idsuratjalan->ViewValue = $this->idsuratjalan->CurrentValue;
            $this->idsuratjalan->ViewValue = FormatNumber($this->idsuratjalan->ViewValue, 0, -2, -2, -2);
            $this->idsuratjalan->ViewCustomAttributes = "";
        } else {
            $this->idsuratjalan->EditValue = $this->idsuratjalan->CurrentValue;
            $this->idsuratjalan->PlaceHolder = RemoveHtml($this->idsuratjalan->caption());
        }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->idsuratjalan);
                    $doc->exportCaption($this->idinvoice);
                    $doc->exportCaption($this->keterangan);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->idsuratjalan);
                        $doc->exportField($this->idinvoice);
                        $doc->exportField($this->keterangan);
    ...
    public function getUserIDSubquery(&$fld, &$masterfld)
    {
        global $UserTable;
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM `suratjalan_detail`";
    ...
    {
        $filterWrk = $filter;
        if ($currentMasterTable == "suratjalan") {
            $filterWrk = Container("suratjalan")->addUserIDFilter($filterWrk);
        }
        return $filterWrk;
    }
    ...
    public function addDetailUserIDFilter($filter, $currentMasterTable)
    {
        $filterWrk = $filter;
        if ($currentMasterTable == "suratjalan") {
            $mastertable = Container("suratjalan");
            if (!$mastertable->userIdAllow()) {
                $subqueryWrk = $mastertable->getUserIDSubquery($this->idsuratjalan, $mastertable->id);
    ...
    public function rowInserting($rsold, &$rsnew)
    {
        ...
        $update = ExecuteUpdate("UPDATE invoice SET sent=1 WHERE id=".$rsnew['idinvoice']);
        return true;
    }
    ...
    public function rowDeleting(&$rs)
    {
        // Enter your code here
        // To cancel, set return value to False
        $update = ExecuteUpdate("UPDATE invoice SET sent=0 WHERE id=".$rs['idinvoice']);
        return true;
    }
