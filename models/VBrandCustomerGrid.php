 ...
 * Page class
 */
class VBrandCustomerGrid extends VBrandCustomer
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'v_brand_customer';

    // Page object name
    public $PageObjName = "VBrandCustomerGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fv_brand_customergrid";
    ...
    public function __construct()
    {
        ...
        // Table object (v_brand_customer)
        if (!isset($GLOBALS["v_brand_customer"]) || get_class($GLOBALS["v_brand_customer"]) == PROJECT_NAMESPACE . "v_brand_customer") {
            $GLOBALS["v_brand_customer"] = &$this;
        }

        // Page URL
        $pageUrl = $this->pageUrl();
        $this->AddUrl = "VBrandCustomerAdd";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'v_brand_customer');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("v_brand_customer"));
                $doc->Text = @$content;
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->idbrand->CurrentValue;
                    if ($key != "") {
                        $key .= Config("COMPOSITE_KEY_SEPARATOR");
                    }
                    $key .= $this->idcustomer->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_kode_brand") && $CurrentForm->hasValue("o_kode_brand") && $this->kode_brand->CurrentValue != $this->kode_brand->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_nama_brand") && $CurrentForm->hasValue("o_nama_brand") && $this->nama_brand->CurrentValue != $this->nama_brand->OldValue) {
            return false;
        }
        ...
        if ($CurrentForm->hasValue("x_jumlah_produk") && $CurrentForm->hasValue("o_jumlah_produk") && $this->jumlah_produk->CurrentValue != $this->jumlah_produk->OldValue) {
            return false;
        }
        return true;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'kode_brand' first before field var 'x_kode_brand'
        $val = $CurrentForm->hasValue("kode_brand") ? $CurrentForm->getValue("kode_brand") : $CurrentForm->getValue("x_kode_brand");
        if (!$this->kode_brand->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->kode_brand->Visible = false; // Disable update for API request
            } else {
                $this->kode_brand->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_kode_brand")) {
            $this->kode_brand->setOldValue($CurrentForm->getValue("o_kode_brand"));
        }
        ...
        if ($CurrentForm->hasValue("o_nama_brand")) {
            $this->nama_brand->setOldValue($CurrentForm->getValue("o_nama_brand"));
        }
        ...
        if (!$this->kode_customer->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->kode_customer->Visible = false; // Disable update for API request
            } else {
                $this->kode_customer->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_kode_customer")) {
            $this->kode_customer->setOldValue($CurrentForm->getValue("o_kode_customer"));
        }
        ...
        // Check field name 'jumlah_produk' first before field var 'x_jumlah_produk'
        $val = $CurrentForm->hasValue("jumlah_produk") ? $CurrentForm->getValue("jumlah_produk") : $CurrentForm->getValue("x_jumlah_produk");
        if (!$this->jumlah_produk->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->jumlah_produk->Visible = false; // Disable update for API request
            } else {
                $this->jumlah_produk->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->kode_brand->CurrentValue = $this->kode_brand->FormValue;
        ...
        $this->jumlah_produk->CurrentValue = $this->jumlah_produk->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->kode_brand->EditValue = HtmlEncode($this->kode_brand->CurrentValue);
            ...
            $this->kode_customer->EditValue = HtmlEncode($this->kode_customer->CurrentValue);
            ...
            $this->jumlah_produk->EditValue = HtmlEncode($this->jumlah_produk->CurrentValue);
            $this->jumlah_produk->PlaceHolder = RemoveHtml($this->jumlah_produk->caption());
    ...
    protected function validateForm()
    {
        ...
        if ($this->kode_brand->Required) {
            if (!$this->kode_brand->IsDetailKey && EmptyValue($this->kode_brand->FormValue)) {
                $this->kode_brand->addErrorMessage(str_replace("%s", $this->kode_brand->caption(), $this->kode_brand->RequiredErrorMessage));
            }
        }
        ...
        if ($this->kode_customer->Required) {
            if (!$this->kode_customer->IsDetailKey && EmptyValue($this->kode_customer->FormValue)) {
                $this->kode_customer->addErrorMessage(str_replace("%s", $this->kode_customer->caption(), $this->kode_customer->RequiredErrorMessage));
            }
        }
        ...
        if ($this->jumlah_produk->Required) {
            if (!$this->jumlah_produk->IsDetailKey && EmptyValue($this->jumlah_produk->FormValue)) {
                $this->jumlah_produk->addErrorMessage(str_replace("%s", $this->jumlah_produk->caption(), $this->jumlah_produk->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->jumlah_produk->FormValue)) {
            $this->jumlah_produk->addErrorMessage($this->jumlah_produk->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // kode_brand
            $this->kode_brand->setDbValueDef($rsnew, $this->kode_brand->CurrentValue, null, $this->kode_brand->ReadOnly);
            ...
            // jumlah_produk
            $this->jumlah_produk->setDbValueDef($rsnew, $this->jumlah_produk->CurrentValue, 0, $this->jumlah_produk->ReadOnly);
    ...
    protected function addRow($rsold = null)
    {
        ...
        // kode_brand
        $this->kode_brand->setDbValueDef($rsnew, $this->kode_brand->CurrentValue, null, false);
        ...
        // jumlah_produk
        $this->jumlah_produk->setDbValueDef($rsnew, $this->jumlah_produk->CurrentValue, 0, strval($this->jumlah_produk->CurrentValue) == "");
