 ...
 * Page class
 */
class IjinhakiStatusList extends IjinhakiStatus
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "IjinhakiStatusList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fijinhaki_statuslist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "IjinhakiStatusAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "IjinhakiStatusDelete";
        $this->MultiUpdateUrl = "IjinhakiStatusUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fijinhaki_statuslistsrch";
    ...
    public function run()
    {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
                if ($this->getCurrentMasterTable() == "ijinhaki") {
                    $this->DbMasterFilter = $this->addMasterUserIDFilter($this->DbMasterFilter, "ijinhaki"); // Add master User ID filter
                }
        }
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "ijinhaki") {
            $masterTbl = Container("ijinhaki");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("IjinhakiList"); // Return to master page
                return;
            } else {
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->idijinhaki->AdvancedSearch->toJson(), ","); // Field idijinhaki
        $filterList = Concat($filterList, $this->idpegawai->AdvancedSearch->toJson(), ","); // Field idpegawai
        $filterList = Concat($filterList, $this->status->AdvancedSearch->toJson(), ","); // Field status
        $filterList = Concat($filterList, $this->targetmulai->AdvancedSearch->toJson(), ","); // Field targetmulai
        $filterList = Concat($filterList, $this->tglmulai->AdvancedSearch->toJson(), ","); // Field tglmulai
        $filterList = Concat($filterList, $this->targetselesai->AdvancedSearch->toJson(), ","); // Field targetselesai
        $filterList = Concat($filterList, $this->tglselesai->AdvancedSearch->toJson(), ","); // Field tglselesai
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fijinhaki_statuslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field idijinhaki
        $this->idijinhaki->AdvancedSearch->SearchValue = @$filter["x_idijinhaki"];
        $this->idijinhaki->AdvancedSearch->SearchOperator = @$filter["z_idijinhaki"];
        $this->idijinhaki->AdvancedSearch->SearchCondition = @$filter["v_idijinhaki"];
        $this->idijinhaki->AdvancedSearch->SearchValue2 = @$filter["y_idijinhaki"];
        $this->idijinhaki->AdvancedSearch->SearchOperator2 = @$filter["w_idijinhaki"];
        $this->idijinhaki->AdvancedSearch->save();
        ...
        // Field targetmulai
        $this->targetmulai->AdvancedSearch->SearchValue = @$filter["x_targetmulai"];
        $this->targetmulai->AdvancedSearch->SearchOperator = @$filter["z_targetmulai"];
        $this->targetmulai->AdvancedSearch->SearchCondition = @$filter["v_targetmulai"];
        $this->targetmulai->AdvancedSearch->SearchValue2 = @$filter["y_targetmulai"];
        $this->targetmulai->AdvancedSearch->SearchOperator2 = @$filter["w_targetmulai"];
        $this->targetmulai->AdvancedSearch->save();
        ...
        // Field targetselesai
        $this->targetselesai->AdvancedSearch->SearchValue = @$filter["x_targetselesai"];
        $this->targetselesai->AdvancedSearch->SearchOperator = @$filter["z_targetselesai"];
        $this->targetselesai->AdvancedSearch->SearchCondition = @$filter["v_targetselesai"];
        $this->targetselesai->AdvancedSearch->SearchValue2 = @$filter["y_targetselesai"];
        $this->targetselesai->AdvancedSearch->SearchOperator2 = @$filter["w_targetselesai"];
        $this->targetselesai->AdvancedSearch->save();

        // Field tglselesai
        $this->tglselesai->AdvancedSearch->SearchValue = @$filter["x_tglselesai"];
        $this->tglselesai->AdvancedSearch->SearchOperator = @$filter["z_tglselesai"];
        $this->tglselesai->AdvancedSearch->SearchCondition = @$filter["v_tglselesai"];
        $this->tglselesai->AdvancedSearch->SearchValue2 = @$filter["y_tglselesai"];
        $this->tglselesai->AdvancedSearch->SearchOperator2 = @$filter["w_tglselesai"];
        $this->tglselesai->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->targetmulai); // targetmulai
            $this->updateSort($this->tglmulai); // tglmulai
            $this->updateSort($this->targetselesai); // targetselesai
            $this->updateSort($this->tglselesai); // tglselesai
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->setSessionOrderBy($orderBy);
                $this->id->setSort("");
                $this->idijinhaki->setSort("");
                $this->idpegawai->setSort("");
                $this->status->setSort("");
                $this->targetmulai->setSort("");
                $this->tglmulai->setSort("");
                $this->targetselesai->setSort("");
                $this->tglselesai->setSort("");
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fijinhaki_statuslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fijinhaki_statuslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fijinhaki_statuslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fijinhaki_statuslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
