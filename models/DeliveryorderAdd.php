 ...
 * Page class
 */
class DeliveryorderAdd extends Deliveryorder
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DeliveryorderAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("DeliveryorderList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "DeliveryorderList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "DeliveryorderView") {
    ...
    protected function validateForm()
    {
        ...
        if (in_array("deliveryorder_detail", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            ...
            $detailPage = Container("DeliveryorderDetailGrid");
            if (in_array("deliveryorder_detail", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->iddeliveryorder->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "deliveryorder_detail"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->iddeliveryorder->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
