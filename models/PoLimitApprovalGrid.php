 ...
 * Page class
 */
class PoLimitApprovalGrid extends PoLimitApproval
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PoLimitApprovalGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fpo_limit_approvalgrid";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "PoLimitApprovalAdd";
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "po_limit_approval_detail") {
            $masterTbl = Container("po_limit_approval_detail");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("PoLimitApprovalDetailList"); // Return to master page
                return;
            } else {
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_limit_kredit") && $CurrentForm->hasValue("o_limit_kredit") && $this->limit_kredit->CurrentValue != $this->limit_kredit->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_limit_po_aktif") && $CurrentForm->hasValue("o_limit_po_aktif") && $this->limit_po_aktif->CurrentValue != $this->limit_po_aktif->OldValue) {
            return false;
        }
        ...
        if ($CurrentForm->hasValue("x_sisalimitkredit") && $CurrentForm->hasValue("o_sisalimitkredit") && $this->sisalimitkredit->CurrentValue != $this->sisalimitkredit->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_sisapoaktif") && $CurrentForm->hasValue("o_sisapoaktif") && $this->sisapoaktif->CurrentValue != $this->sisapoaktif->OldValue) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->limit_kredit->clearErrorMessage();
        $this->limit_po_aktif->clearErrorMessage();
        $this->created_at->clearErrorMessage();
        $this->sisalimitkredit->clearErrorMessage();
        $this->sisapoaktif->clearErrorMessage();
    }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_limit_kredit")) {
            $this->limit_kredit->setOldValue($CurrentForm->getValue("o_limit_kredit"));
        }
        ...
        if ($CurrentForm->hasValue("o_limit_po_aktif")) {
            $this->limit_po_aktif->setOldValue($CurrentForm->getValue("o_limit_po_aktif"));
        }
        ...
        if (!$this->created_at->IsDetailKey) {
            ...
            $this->created_at->CurrentValue = UnFormatDateTime($this->created_at->CurrentValue, 1);
        }
        ...
        if ($CurrentForm->hasValue("o_sisalimitkredit")) {
            $this->sisalimitkredit->setOldValue($CurrentForm->getValue("o_sisalimitkredit"));
        }
        ...
        if ($CurrentForm->hasValue("o_sisapoaktif")) {
            $this->sisapoaktif->setOldValue($CurrentForm->getValue("o_sisapoaktif"));
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->sisalimitkredit->EditValue = HtmlEncode($this->sisalimitkredit->CurrentValue);
            $this->sisalimitkredit->PlaceHolder = RemoveHtml($this->sisalimitkredit->caption());

            // sisapoaktif
            $this->sisapoaktif->EditAttrs["class"] = "form-control";
            $this->sisapoaktif->EditCustomAttributes = "";
            $this->sisapoaktif->EditValue = HtmlEncode($this->sisapoaktif->CurrentValue);
            $this->sisapoaktif->PlaceHolder = RemoveHtml($this->sisapoaktif->caption());
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // created_at
            $this->created_at->setDbValueDef($rsnew, UnFormatDateTime($this->created_at->CurrentValue, 1), null, $this->created_at->ReadOnly);
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "po_limit_approval_detail") {
        ...
        // created_at
        $this->created_at->setDbValueDef($rsnew, UnFormatDateTime($this->created_at->CurrentValue, 1), null, false);

        // sisalimitkredit
        $this->sisalimitkredit->setDbValueDef($rsnew, $this->sisalimitkredit->CurrentValue, null, false);

        // sisapoaktif
        $this->sisapoaktif->setDbValueDef($rsnew, $this->sisapoaktif->CurrentValue, null, false);
    ...
    protected function setupMasterParms()
    {
        ...
        if ($masterTblVar == "po_limit_approval_detail") {
            $masterTbl = Container("po_limit_approval_detail");
            $this->id->Visible = false;
