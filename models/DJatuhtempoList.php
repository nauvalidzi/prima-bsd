 ...
 * Page class
 */
class DJatuhtempoList extends DJatuhtempo
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'd_jatuhtempo';

    // Page object name
    public $PageObjName = "DJatuhtempoList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fd_jatuhtempolist";
    ...
    public function __construct()
    {
        ...
        // Table object (d_jatuhtempo)
        if (!isset($GLOBALS["d_jatuhtempo"]) || get_class($GLOBALS["d_jatuhtempo"]) == PROJECT_NAMESPACE . "d_jatuhtempo") {
            $GLOBALS["d_jatuhtempo"] = &$this;
        }
        ...
        $this->AddUrl = "DJatuhtempoAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "DJatuhtempoDelete";
        $this->MultiUpdateUrl = "DJatuhtempoUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'd_jatuhtempo');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fd_jatuhtempolistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("d_jatuhtempo"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->namapegawai->setVisibility();
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fd_jatuhtempolistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field sisahari
        $this->sisahari->AdvancedSearch->SearchValue = @$filter["x_sisahari"];
        $this->sisahari->AdvancedSearch->SearchOperator = @$filter["z_sisahari"];
        $this->sisahari->AdvancedSearch->SearchCondition = @$filter["v_sisahari"];
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->namapegawai, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->namacustomer, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->kodeinvoice, $arKeywords, $type);
        return $where;
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->namapegawai); // namapegawai
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fd_jatuhtempolistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fd_jatuhtempolistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fd_jatuhtempolist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fd_jatuhtempolistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
