 ...
 * Page class
 */
class BrandAddopt extends Brand
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "BrandAddopt";
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->idcustomer->IsDetailKey) {
            $this->idcustomer->setFormValue(ConvertFromUtf8($val));
        }
        ...
        if (!$this->kode->IsDetailKey) {
            $this->kode->setFormValue(ConvertFromUtf8($val));
        }
        ...
        if (!$this->titipmerk->IsDetailKey) {
            $this->titipmerk->setFormValue(ConvertFromUtf8($val));
        }
        ...
        if (!$this->ijinhaki->IsDetailKey) {
            $this->ijinhaki->setFormValue(ConvertFromUtf8($val));
        }
        ...
        if (!$this->ijinbpom->IsDetailKey) {
            $this->ijinbpom->setFormValue(ConvertFromUtf8($val));
        }

        // Check field name 'created_at' first before field var 'x_created_at'
        $val = $CurrentForm->hasValue("created_at") ? $CurrentForm->getValue("created_at") : $CurrentForm->getValue("x_created_at");
        if (!$this->created_at->IsDetailKey) {
            $this->created_at->setFormValue(ConvertFromUtf8($val));
    ...
    {
        global $CurrentForm;
        $this->idcustomer->CurrentValue = ConvertToUtf8($this->idcustomer->FormValue);
        $this->title->CurrentValue = ConvertToUtf8($this->title->FormValue);
        $this->kode->CurrentValue = ConvertToUtf8($this->kode->FormValue);
        $this->titipmerk->CurrentValue = ConvertToUtf8($this->titipmerk->FormValue);
        $this->ijinhaki->CurrentValue = ConvertToUtf8($this->ijinhaki->FormValue);
        $this->ijinbpom->CurrentValue = ConvertToUtf8($this->ijinbpom->FormValue);
        $this->created_at->CurrentValue = ConvertToUtf8($this->created_at->FormValue);
        $this->created_at->CurrentValue = UnFormatDateTime($this->created_at->CurrentValue, 0);
