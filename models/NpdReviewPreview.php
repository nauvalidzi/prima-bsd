 ...
 * Page class
 */
class NpdReviewPreview extends NpdReview
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "NpdReviewPreview";
    ...
    public function pageDataRendering(&$header)
    {
        // Example:
        //$header = "your header";
        $sample = ExecuteScalar("SELECT COUNT(*) FROM npd_sample WHERE `status`=0 AND idnpd=".$this->idnpd->CurrentValue);
        $selesai = ExecuteScalar("SELECT selesai FROM npd WHERE id=".$this->idnpd->CurrentValue);
        if(empty($sample) || $selesai) {
