 ...
 * Page class
 */
class VInvoiceNotCreatedList extends VInvoiceNotCreated
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'v_invoice_not_created';

    // Page object name
    public $PageObjName = "VInvoiceNotCreatedList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fv_invoice_not_createdlist";
    ...
    public function __construct()
    {
        ...
        // Table object (v_invoice_not_created)
        if (!isset($GLOBALS["v_invoice_not_created"]) || get_class($GLOBALS["v_invoice_not_created"]) == PROJECT_NAMESPACE . "v_invoice_not_created") {
            $GLOBALS["v_invoice_not_created"] = &$this;
        }
        ...
        $this->AddUrl = "VInvoiceNotCreatedAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "VInvoiceNotCreatedDelete";
        $this->MultiUpdateUrl = "VInvoiceNotCreatedUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'v_invoice_not_created');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fv_invoice_not_createdlistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("v_invoice_not_created"));
                $doc->Text = @$content;
    ...
    protected function getRecordKeyValue($ar)
    {
        $key = "";
        if (is_array($ar)) {
            $key .= @$ar['idorder'] . Config("COMPOSITE_KEY_SEPARATOR");
            $key .= @$ar['idcustomer'];
        }
        return $key;
    }
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fv_invoice_not_createdlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field tanggalorder
        $this->tanggalorder->AdvancedSearch->SearchValue = @$filter["x_tanggalorder"];
        $this->tanggalorder->AdvancedSearch->SearchOperator = @$filter["z_tanggalorder"];
        $this->tanggalorder->AdvancedSearch->SearchCondition = @$filter["v_tanggalorder"];
        $this->tanggalorder->AdvancedSearch->SearchValue2 = @$filter["y_tanggalorder"];
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->idorder->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->idcustomer->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fv_invoice_not_createdlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fv_invoice_not_createdlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fv_invoice_not_createdlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fv_invoice_not_createdlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
