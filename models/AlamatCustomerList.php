 ...
 * Page class
 */
class AlamatCustomerList extends AlamatCustomer
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AlamatCustomerList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "falamat_customerlist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "AlamatCustomerDelete";
        $this->MultiUpdateUrl = "AlamatCustomerUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option falamat_customerlistsrch";
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "falamat_customerlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->idprovinsi->AdvancedSearch->SearchCondition = @$filter["v_idprovinsi"];
        $this->idprovinsi->AdvancedSearch->SearchValue2 = @$filter["y_idprovinsi"];
        $this->idprovinsi->AdvancedSearch->SearchOperator2 = @$filter["w_idprovinsi"];
        $this->idprovinsi->AdvancedSearch->save();

        // Field idkabupaten
        $this->idkabupaten->AdvancedSearch->SearchValue = @$filter["x_idkabupaten"];
        $this->idkabupaten->AdvancedSearch->SearchOperator = @$filter["z_idkabupaten"];
        $this->idkabupaten->AdvancedSearch->SearchCondition = @$filter["v_idkabupaten"];
        $this->idkabupaten->AdvancedSearch->SearchValue2 = @$filter["y_idkabupaten"];
        $this->idkabupaten->AdvancedSearch->SearchOperator2 = @$filter["w_idkabupaten"];
        $this->idkabupaten->AdvancedSearch->save();

        // Field idkecamatan
        $this->idkecamatan->AdvancedSearch->SearchValue = @$filter["x_idkecamatan"];
        $this->idkecamatan->AdvancedSearch->SearchOperator = @$filter["z_idkecamatan"];
        $this->idkecamatan->AdvancedSearch->SearchCondition = @$filter["v_idkecamatan"];
        $this->idkecamatan->AdvancedSearch->SearchValue2 = @$filter["y_idkecamatan"];
        $this->idkecamatan->AdvancedSearch->SearchOperator2 = @$filter["w_idkecamatan"];
        $this->idkecamatan->AdvancedSearch->save();

        // Field idkelurahan
        $this->idkelurahan->AdvancedSearch->SearchValue = @$filter["x_idkelurahan"];
        $this->idkelurahan->AdvancedSearch->SearchOperator = @$filter["z_idkelurahan"];
        $this->idkelurahan->AdvancedSearch->SearchCondition = @$filter["v_idkelurahan"];
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"falamat_customerlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"falamat_customerlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.falamat_customerlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"falamat_customerlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
