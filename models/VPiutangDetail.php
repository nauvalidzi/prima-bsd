...
/**
 * Table class for v_piutang_detail
 */
class VPiutangDetail extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'v_piutang_detail';
        $this->TableName = 'v_piutang_detail';
        $this->TableType = 'VIEW';

        // Update Table
        $this->UpdateTable = "`v_piutang_detail`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // idinvoice
        $this->idinvoice = new DbField('v_piutang_detail', 'v_piutang_detail', 'x_idinvoice', 'idinvoice', '`idinvoice`', '`idinvoice`', 20, 20, -1, false, '`idinvoice`', false, false, false, 'FORMATTED TEXT', 'NO');
    ...
    // Master filter
    public function sqlMasterFilter_v_piutang()
    {
        return "`idcustomer`=@idcustomer@";
    }
    // Detail filter
    public function sqlDetailFilter_v_piutang()
    {
        return "`idcustomer`=@idcustomer@";
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`v_piutang_detail`";
    }
    ...
    public function applyUserIDFilters($filter)
    {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
            if ($this->getCurrentMasterTable() == "v_piutang" || $this->getCurrentMasterTable() == "") {
                $filter = $this->addDetailUserIDFilter($filter, "v_piutang"); // Add detail User ID filter
            }
        }
        return $filter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("VPiutangDetailList");
    }
    ...
    {
        global $Language;
        if ($pageName == "VPiutangDetailView") {
            return $Language->phrase("View");
        } elseif ($pageName == "VPiutangDetailEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "VPiutangDetailAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "VPiutangDetailView";
            case Config("API_ADD_ACTION"):
                return "VPiutangDetailAdd";
            case Config("API_EDIT_ACTION"):
                return "VPiutangDetailEdit";
            case Config("API_DELETE_ACTION"):
                return "VPiutangDetailDelete";
            case Config("API_LIST_ACTION"):
                return "VPiutangDetailList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("VPiutangDetailView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("VPiutangDetailView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "VPiutangDetailAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "VPiutangDetailAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("VPiutangDetailEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("VPiutangDetailAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("VPiutangDetailDelete", $this->getUrlParm());
    }

    // Add master url
    public function addMasterUrl($url)
    {
        if ($this->getCurrentMasterTable() == "v_piutang" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
...
SORTHTML;
    ...
    {
        $filterWrk = $filter;
        if ($currentMasterTable == "v_piutang") {
            $filterWrk = Container("v_piutang")->addUserIDFilter($filterWrk);
        }
        return $filterWrk;
    }
    ...
    public function addDetailUserIDFilter($filter, $currentMasterTable)
    {
        $filterWrk = $filter;
        if ($currentMasterTable == "v_piutang") {
            $mastertable = Container("v_piutang");
