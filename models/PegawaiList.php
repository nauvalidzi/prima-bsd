...
class PegawaiList extends Pegawai
{
    ...
    {
                $this->pid->AdvancedSearch->loadDefault();
        return true;
    }
    ...
    public function renderListOptions()
    {
        ...
        if ($Security->allowList(CurrentProjectID() . 'customer') && $this->showOptionLink()) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("customer", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("CustomerList?" . Config("TABLE_SHOW_MASTER") . "=pegawai&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("CustomerGrid");
            if ($detailPage->DetailView && $Security->canView() && $this->showOptionLink("view") && $Security->allowView(CurrentProjectID() . 'pegawai')) {
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $this->showOptionLink("edit") && $Security->allowEdit(CurrentProjectID() . 'pegawai')) {
    ...
    protected function renderListOptionsExt()
    {
        ...
        $links = "";
        $btngrps = "";
        $sqlwrk = "`idpegawai`=" . AdjustSql($this->id->CurrentValue, $this->Dbid) . "";
        ...
        if ($this->DetailPages && $this->DetailPages["customer"] && $this->DetailPages["customer"]->Visible) {
            ...
            $url = "CustomerPreview?t=pegawai&f=" . Encrypt($sqlwrk);
            ...
            if ($Security->allowList(CurrentProjectID() . 'pegawai')) {
                ...
                $links .= $link;
                $detaillnk = JsEncodeAttribute("CustomerList?" . Config("TABLE_SHOW_MASTER") . "=pegawai&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $this->showOptionLink("view") && $Security->allowView(CurrentProjectID() . 'pegawai')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=customer");
                $btngrp .= "<a href=\"#\" class=\"mr-2\" title=\"" . HtmlTitle($caption) . "\" onclick=\"window.location='" . HtmlEncode($url) . "';return false;\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $this->showOptionLink("edit") && $Security->allowEdit(CurrentProjectID() . 'pegawai')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=customer");
                $btngrp .= "<a href=\"#\" class=\"mr-2\" title=\"" . HtmlTitle($caption) . "\" onclick=\"window.location='" . HtmlEncode($url) . "';return false;\">" . $caption . "</a>";
            }
