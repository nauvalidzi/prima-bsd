 ...
 * Page class
 */
class IjinbpomDetailList extends IjinbpomDetail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "IjinbpomDetailList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fijinbpom_detaillist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "IjinbpomDetailAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "IjinbpomDetailDelete";
        $this->MultiUpdateUrl = "IjinbpomDetailUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fijinbpom_detaillistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->namaalt->AdvancedSearch->toJson(), ","); // Field namaalt
        ...
        $filterList = Concat($filterList, $this->kodesample->AdvancedSearch->toJson(), ","); // Field kodesample
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fijinbpom_detaillistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field namaalt
        $this->namaalt->AdvancedSearch->SearchValue = @$filter["x_namaalt"];
        $this->namaalt->AdvancedSearch->SearchOperator = @$filter["z_namaalt"];
        $this->namaalt->AdvancedSearch->SearchCondition = @$filter["v_namaalt"];
        $this->namaalt->AdvancedSearch->SearchValue2 = @$filter["y_namaalt"];
        $this->namaalt->AdvancedSearch->SearchOperator2 = @$filter["w_namaalt"];
        $this->namaalt->AdvancedSearch->save();
        ...
        // Field kodesample
        $this->kodesample->AdvancedSearch->SearchValue = @$filter["x_kodesample"];
        $this->kodesample->AdvancedSearch->SearchOperator = @$filter["z_kodesample"];
        $this->kodesample->AdvancedSearch->SearchCondition = @$filter["v_kodesample"];
        $this->kodesample->AdvancedSearch->SearchValue2 = @$filter["y_kodesample"];
        $this->kodesample->AdvancedSearch->SearchOperator2 = @$filter["w_kodesample"];
        $this->kodesample->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->namaalt, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->ukuran, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->kodesample, $arKeywords, $type);
        return $where;
    }
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->idnpd->setSort("");
                $this->nama->setSort("");
                $this->namaalt->setSort("");
                $this->idproduct_acuan->setSort("");
                $this->ukuran->setSort("");
                $this->kodesample->setSort("");
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fijinbpom_detaillistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fijinbpom_detaillistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fijinbpom_detaillist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fijinbpom_detaillistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    public function pageDataRendered(&$footer)
    {
        ...
        function selesai(id) {
            var nama = prompt('Masukkan nama yang disetujui:');
            if (nama == null || nama == '') {
            	alert('Nama tidak boleh kosong');
            } else {
            	$.get('/bsd/api/ijinbpom_detail/selesai/'+id+'/'+nama, function(data) {
    				if (data == 1) {
    					location.reload();
    				} else {
        ...
        function belumselesai(id) {
    		$.get('/bsd/api/ijinbpom_detail/belumselesai/'+id, function(data) {
    			if (data == 1) {
    				location.reload();
    			} else {
