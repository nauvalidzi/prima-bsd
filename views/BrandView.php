...
<?php } ?>
<?php
    if (in_array("v_list_brand_customers", explode(",", $Page->getCurrentDetailTable())) && $v_list_brand_customers->DetailView) {
...
?>
    <div class="card ew-accordion-card <?= $Page->DetailPages->pageStyle("v_list_brand_customers") ?>">
        <div class="card-header">
            <h4 class="card-title">
                <a data-toggle="collapse" role="button" class="collapsed" aria-expanded="<?= JsonEncode($Page->DetailPages->isActive("v_list_brand_customers")) ?>" data-parent="#Page_details" href="#tab_v_list_brand_customers"><?= $Language->tablePhrase("v_list_brand_customers", "TblCaption") ?>&nbsp;<?= str_replace("%c", Container("v_list_brand_customers")->Count, $Language->phrase("DetailCount")) ?></a>
            </h4>
        </div>
