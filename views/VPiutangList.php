...
loadjs.ready("head", function () {
    ...
    fv_piutanglist = currentForm = new ew.Form("fv_piutanglist", "list");
...
<div class="card ew-card ew-grid<?php if ($Page->isAddOrEdit()) { ?> ew-grid-add-edit<?php } ?> v_piutang">
<form name="fv_piutanglist" id="fv_piutanglist" class="form-inline ew-form ew-list-form" action="<?= CurrentPageUrl(false) ?>" method="post">
<?php if (Config("CHECK_TOKEN")) { ?>
<input type="hidden" name="<?= $TokenNameKey ?>" value="<?= $TokenName ?>"><!-- CSRF token name -->
<input type="hidden" name="<?= $TokenValueKey ?>" value="<?= $TokenValue ?>"><!-- CSRF token value -->
<?php } ?>
<input type="hidden" name="t" value="v_piutang">
<div id="gmp_v_piutang" class="<?= ResponsiveTableClass() ?>card-body ew-grid-middle-panel">
<?php if ($Page->TotalRecords > 0 || $Page->isGridEdit()) { ?>
<table id="tbl_v_piutanglist" class="table ew-table"><!-- .ew-table -->
<thead>
...
<?php if ($Page->totalpiutang->Visible) { // totalpiutang ?>
        <th data-name="totalpiutang" class="<?= $Page->totalpiutang->headerCellClass() ?>"><div id="elh_v_piutang_totalpiutang" class="v_piutang_totalpiutang"><?= $Page->renderSort($Page->totalpiutang) ?></div></th>
<?php } ?>
<?php
...
while ($Page->RecordCount < $Page->StopRecord) {
    ...
    if ($Page->RecordCount >= $Page->StartRecord) {
        ...
        $Page->RowAttrs->merge(["data-rowindex" => $Page->RowCount, "id" => "r" . $Page->RowCount . "_v_piutang", "data-rowtype" => $Page->RowType]);
...
<span id="el<?= $Page->RowCount ?>_v_piutang_totaltagihan">
...
<?= $Page->totaltagihan->getViewValue() ?></span>
    ...
    <?php } ?>
    <?php if ($Page->totalpiutang->Visible) { // totalpiutang ?>
        <td data-name="totalpiutang" <?= $Page->totalpiutang->cellAttributes() ?>>
<span id="el<?= $Page->RowCount ?>_v_piutang_totalpiutang">
<span<?= $Page->totalpiutang->viewAttributes() ?>>
<?= $Page->totalpiutang->getViewValue() ?></span>
</span>
</td>
    <?php } ?>
<?php
