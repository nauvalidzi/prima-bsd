...
// Page object
$DeliveryorderEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fdeliveryorderedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fdeliveryorderedit = currentForm = new ew.Form("fdeliveryorderedit", "edit");
    ...
    // Validate form
    fdeliveryorderedit.validate = function () {
    ...
    // Form_CustomValidate
    fdeliveryorderedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fdeliveryorderedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fdeliveryorderedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fdeliveryorderedit" id="fdeliveryorderedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready(["fdeliveryorderedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fdeliveryorderedit", "x_tanggal", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<?php } ?>
    ...
    <input type="hidden" data-table="deliveryorder" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php
    if (in_array("deliveryorder_detail", explode(",", $Page->getCurrentDetailTable())) && $deliveryorder_detail->DetailEdit) {
?>
