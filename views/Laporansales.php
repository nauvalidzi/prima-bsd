...
<?php
	...
	$sj= ExecuteRow("SELECT sj.*, c.nama as namacustomer, c.hp as handphone, c.alamat, kel.nama kel, kec.nama kec, kab.nama kab, prov.name prov FROM suratjalan sj, customer c LEFT JOIN kelurahan kel ON c.idkel=kel.id LEFT JOIN kecamatan kec ON c.idkec=kec.id LEFT JOIN kabupaten kab ON c.idkab=kab.id LEFT JOIN provinsi prov ON c.idprov=prov.id WHERE sj.idcustomer=c.id and sj.id=".$idSj);
...
<html lang="en">
    ...
    <body>
        ...
        <div style="border: 1px solid; padding: 1em;">
            <p><?= $sj['namacustomer'] ?><?php echo !empty($sj['handphone']) ? ", {$sj['handphone']}" : null ?><br>
            <p><?php
                if (!empty($sj['alamat'])) {
                    echo "{$sj['alamat']}<br>";
                }
