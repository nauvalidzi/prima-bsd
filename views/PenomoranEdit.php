...
loadjs.ready("head", function () {
    ...
    fpenomoranedit = currentForm = new ew.Form("fpenomoranedit", "edit");
    ...
    // Validate form
    fpenomoranedit.validate = function () {
    ...
    // Form_CustomValidate
    fpenomoranedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpenomoranedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fpenomoranedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpenomoranedit" id="fpenomoranedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->display->getDisplayValue($Page->display->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="penomoran" data-field="x_display" data-hidden="1" name="x_display" id="x_display" value="<?= HtmlEncode($Page->display->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
</div><!-- /page* -->
    <input type="hidden" data-table="penomoran" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
