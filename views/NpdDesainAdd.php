...
loadjs.ready("head", function () {
    ...
    fnpd_desainadd = currentForm = new ew.Form("fnpd_desainadd", "add");
    ...
    // Validate form
    fnpd_desainadd.validate = function () {
    ...
    // Form_CustomValidate
    fnpd_desainadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fnpd_desainadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fnpd_desainadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fnpd_desainadd" id="fnpd_desainadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready(["fnpd_desainadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fnpd_desainadd", "x_tglterima", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span></template>
...
<script>
loadjs.ready(["fnpd_desainadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fnpd_desainadd", "x_tglsubmit", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span></template>
...
<?php if ($Page->nama_produk->Visible) { // nama_produk ?>
    <div id="r_nama_produk" class="form-group row">
        <label id="elh_npd_desain_nama_produk" for="x_nama_produk" class="<?= $Page->LeftColumnClass ?>"><template id="tpc_npd_desain_nama_produk"><?= $Page->nama_produk->caption() ?><?= $Page->nama_produk->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></template></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->nama_produk->cellAttributes() ?>>
<template id="tpx_npd_desain_nama_produk"><span id="el_npd_desain_nama_produk">
<input type="<?= $Page->nama_produk->getInputTextType() ?>" data-table="npd_desain" data-field="x_nama_produk" name="x_nama_produk" id="x_nama_produk" size="30" maxlength="255" placeholder="<?= HtmlEncode($Page->nama_produk->getPlaceHolder()) ?>" value="<?= $Page->nama_produk->EditValue ?>"<?= $Page->nama_produk->editAttributes() ?> aria-describedby="x_nama_produk_help">
<?= $Page->nama_produk->getCustomMessage() ?>
<div class="invalid-feedback"><?= $Page->nama_produk->getErrorMessage() ?></div>
</span></template>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fnpd_desainadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fnpd_desainadd", "x_created_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span></template>
...
<script>
loadjs.ready(["fnpd_desainadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fnpd_desainadd", "x_updated_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span></template>
...
<div id="tpd_npd_desainadd" class="ew-custom-template"></div>
<template id="tpm_npd_desainadd">
<div id="ct_NpdDesainAdd"><div class="form-horizontal">
    <div class="card">
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_npd_desainadd", "tpm_npd_desainadd", "npd_desainadd", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
