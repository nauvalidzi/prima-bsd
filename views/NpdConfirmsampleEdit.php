...
// Page object
$NpdConfirmsampleEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fnpd_confirmsampleedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fnpd_confirmsampleedit = currentForm = new ew.Form("fnpd_confirmsampleedit", "edit");
    ...
    fnpd_confirmsampleedit.addFields([
    ...
    // Validate form
    fnpd_confirmsampleedit.validate = function () {
    ...
    // Form_CustomValidate
    fnpd_confirmsampleedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fnpd_confirmsampleedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fnpd_confirmsampleedit.lists.bentuk = <?= $Page->bentuk->toClientList($Page) ?>;
    fnpd_confirmsampleedit.lists.viskositas = <?= $Page->viskositas->toClientList($Page) ?>;
    fnpd_confirmsampleedit.lists.warna = <?= $Page->warna->toClientList($Page) ?>;
    fnpd_confirmsampleedit.lists.bauparfum = <?= $Page->bauparfum->toClientList($Page) ?>;
    fnpd_confirmsampleedit.lists.aplikasisediaan = <?= $Page->aplikasisediaan->toClientList($Page) ?>;
    fnpd_confirmsampleedit.lists.receipt_by = <?= $Page->receipt_by->toClientList($Page) ?>;
    loadjs.done("fnpd_confirmsampleedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fnpd_confirmsampleedit" id="fnpd_confirmsampleedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idnpd->getDisplayValue($Page->idnpd->EditValue))) ?>"></span>
</span></template>
<input type="hidden" data-table="npd_confirmsample" data-field="x_idnpd" data-hidden="1" name="x_idnpd" id="x_idnpd" value="<?= HtmlEncode($Page->idnpd->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fnpd_confirmsampleedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fnpd_confirmsampleedit", "x_tglkonfirmasi", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span></template>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idnpd_sample->getDisplayValue($Page->idnpd_sample->EditValue))) ?>"></span>
</span></template>
<input type="hidden" data-table="npd_confirmsample" data-field="x_idnpd_sample" data-hidden="1" name="x_idnpd_sample" id="x_idnpd_sample" value="<?= HtmlEncode($Page->idnpd_sample->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<?php } ?>
    ...
    <input type="hidden" data-table="npd_confirmsample" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<div id="tpd_npd_confirmsampleedit" class="ew-custom-template"></div>
<template id="tpm_npd_confirmsampleedit">
<div id="ct_NpdConfirmsampleEdit"><div class="form-horizontal">
    <div class="card">
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_npd_confirmsampleedit", "tpm_npd_confirmsampleedit", "npd_confirmsampleedit", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
